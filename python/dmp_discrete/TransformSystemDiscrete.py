...
class TransformSystemDiscrete(TransformationSystem, object):
  ...
  def __init__(self,
               ...
               name=""):
    super(TransformSystemDiscrete,
          self).__init__(dmp_num_dimensions_init, canonical_system_discrete,
                         func_approximator_discrete, start_dmpstate_discrete,
                         current_dmpstate_discrete,
                         current_velocity_dmpstate_discrete,
                         goal_system_discrete, transform_couplers_list, name)
    self.alpha = ts_alpha
    self.beta = ts_beta
  ...
  def start(self, start_state_init, goal_state_init):
    assert (self.isValid()), (
        "Pre-condition(s) checking is failed: this TransformSystemDiscrete is "
        "invalid!")
    ...
    if (self.canonical_sys.order == 2):
      ...
      g0 = ((((tau * tau * xdd0) * 1.0 / self.alpha) +
             (tau * xd0)) * 1.0 / self.beta) + x0
    ...
    assert (self.isValid()), (
        "Post-condition(s) checking is failed: this TransformSystemDiscrete "
        "became invalid!")
    return None
  ...
  def getGoalTrajAndCanonicalTrajAndTauAndALearnFromDemo(
      self,
      dmptrajectory_demo_local,
      robot_task_servo_rate,
      steady_state_goal_position_local=None):
    assert (self.isValid()), (
        "Pre-condition(s) checking is failed: this TransformSystemDiscrete is "
        "invalid!")
    assert (dmptrajectory_demo_local.isValid())
    assert (
        dmptrajectory_demo_local.dmp_num_dimensions == self.dmp_num_dimensions)
    ...
    if (steady_state_goal_position_local is None):
      goal_steady_dmpstate_demo_local = dmptrajectory_demo_local.getDMPStateAtIndex(
          traj_length - 1)
    else:
      goal_steady_dmpstate_demo_local = DMPState(
          steady_state_goal_position_local)
    ...
    dt = (goal_steady_dmpstate_demo_local.time[0, 0] -
          start_dmpstate_demo_local.time[0, 0]) / (
              traj_length - 1.0)
    if (dt <= 0.0):
      dt = 1.0 / robot_task_servo_rate
    tau = goal_steady_dmpstate_demo_local.time[
        0, 0] - start_dmpstate_demo_local.time[0, 0]
    if (tau < MIN_TAU):
  ...
  def getTargetForcingTermTraj(self, dmptrajectory_demo_local,
                               robot_task_servo_rate):
    G, cX, cV, tau, tau_relative, A_learn = self.getGoalTrajAndCanonicalTrajAndTauAndALearnFromDemo(
    ...
    F_target = ((np.square(tau_relative) * Tdd) -
                (self.alpha * ((self.beta * (G - T)) - (tau_relative * Td))))

    return F_target, cX, cV, tau, tau_relative, A_learn, G

  def getTargetCouplingTermTraj(self, dmptrajectory_demo_local,
                                robot_task_servo_rate,
                                steady_state_goal_position_local):
    G, cX, cV, tau, tau_relative, A_learn = self.getGoalTrajAndCanonicalTrajAndTauAndALearnFromDemo(
        dmptrajectory_demo_local, robot_task_servo_rate,
    ...
    C_target = ((np.square(tau_relative) * Tdd) - (self.alpha *
                                                   ((self.beta * (G - T)) -
                                                    (tau_relative * Td))) - F)
