...
def dmp_1D_test(dmp_home_dir_path='../../../',
                canonical_order=2,
                time_reproduce_max=0.0,
                time_goal_change=0.0,
                new_goal_scalar=0.0,
                tau_reproduce=0.0,
                unroll_traj_save_dir_path='',
                unroll_traj_save_filename='',
  ...
  set_1Dtraj_input = dmp_discrete_1D.extractSetTrajectories(
      dmp_home_dir_path + '/data/dmp_1D/sample_traj_1.txt')

  [critical_states_learn, W, mean_A_learn, mean_tau, Ft, Fp, G, cX, cV, PSI
  ] = dmp_discrete_1D.learnFromSetTrajectories(
      set_1Dtraj_input,
      task_servo_rate,
  ...
  for i in range(unroll_traj_length):
    ...
    [dmpstate, _, _, _, _] = dmp_discrete_1D.getNextState(dt, True)
  ...
  if (os.path.isdir(unroll_traj_save_dir_path)):
    np.savetxt(unroll_traj_save_dir_path + '/' + unroll_traj_save_filename,
               unroll_traj)

  if (is_plotting):
    plt.close('all')

    dmp1D_unroll = dmp_discrete_1D.unroll(critical_states_learn, tau,
                                          time_reproduce_max, dt)

    dmp_discrete_1D.plotDemosVsUnroll(
        set_1Dtraj_input, dmp1D_unroll, fig_num_offset=10)
