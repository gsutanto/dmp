...
def pi2_test():
    ...
    # initialization:
    position_mean = start
    position_cov = np.diag(np.ones(D) * init_std)
    ...
    for it in range(0, N_iter):
        ...
        print ("iter %d: position_mean = " % it + str(position_mean) + ", cost = %f" % position_mean_cost)
        
        position_samples = np.random.multivariate_normal(position_mean, position_cov, N_sample)
