...
class QuaternionDMPState(DMPState, object):
  ...
  def __init__(self,
    ...
    if ((Q_init is None) and (Qd_init is None) and (Qdd_init is None) and
      ...
      if (Q_init is not None):
        assert (Q_init.shape[0] == 4), "Dimension Q_init.shape[0]=" + str(
            Q_init.shape[0]) + " is NOT 4 (invalid Quaternion)!"
        self.X = Q_init
      else:
      ...
      if (Qd_init is not None):
        assert (Qd_init.shape[0] == 4), "Dimension Qd_init.shape[0]=" + str(
            Qd_init.shape[0]) + " is NOT 4!"
        self.Xd = Qd_init
      else:
      ...
      if (Qdd_init is not None):
        assert (Qdd_init.shape[0] == 4), "Dimension Qdd_init.shape[0]=" + str(
            Qdd_init.shape[0]) + " is NOT 4!"
        self.Xdd = Qdd_init
      else:
      ...
      if (omega_init is not None):
        assert (omega_init.shape[0] == self.dmp_num_dimensions
               ), "Dimension omega_init.shape[0]=" + str(
                   omega_init.shape[0]
      ...
      if (omegad_init is not None):
        assert (omegad_init.shape[0] == self.dmp_num_dimensions
               ), "Dimension omegad_init.shape[0]=" + str(
                   omegad_init.shape[0]
               ) + " is mis-matched with self.dmp_num_dimensions=" + str(
                   self.dmp_num_dimensions) + "!"
  ...
  def isValid(self):
    assert (self.dmp_num_dimensions == 3), "self.dmp_num_dimensions =" + str(
        self.dmp_num_dimensions)
    ...
    assert (len(
        self.omega.shape) == 2), "self.omega.shape =" + str(self.omega.shape)
    assert (len(
        self.omegad.shape) == 2), "self.omegad.shape =" + str(self.omegad.shape)
    assert (len(
        self.time.shape) == 2), "self.time.shape =" + str(self.time.shape)
    assert (self.X.shape[0] == 4), "Dimension self.X.shape[0]=" + str(
        self.X.shape[0]) + " is NOT 4 (invalid Quaternion)!"
    assert (npla.norm(self.X, ord=2, axis=0) >
            0.0).all(), "npla.norm(self.X, ord=2, axis=0) =" + str(
                npla.norm(self.X, ord=2, axis=0))
    assert (self.Xd.shape[0] == 4), "Dimension self.Xd.shape[0]=" + str(
        self.Xd.shape[0]) + " is NOT 4!"
    assert (self.Xdd.shape[0] == 4), "Dimension self.Xdd.shape[0]=" + str(
        self.Xdd.shape[0]) + " is NOT 4!"
    assert (self.omega.shape[0] == self.dmp_num_dimensions
           ), "Dimension self.omega.shape[0]=" + str(
               self.omega.shape[0]
           ) + " is mis-matched with self.dmp_num_dimensions=" + str(
               self.dmp_num_dimensions) + "!"
    assert (self.omegad.shape[0] == self.dmp_num_dimensions
           ), "Dimension self.omegad.shape[0]=" + str(
               self.omegad.shape[0]
           ) + " is mis-matched with self.dmp_num_dimensions=" + str(
               self.dmp_num_dimensions) + "!"
    ...
    assert (np.amin(self.time) >= 0.0), "min(self.time)=" + str(
        np.amin(self.time)) + " < 0.0 (invalid!)"
    return True
  ...
  def setQ(self, new_Q):
    assert (self.isValid()), (
        "Pre-condition(s) checking is failed: this QuaternionDMPState is "
        "invalid!")
    assert (new_Q.shape[0] == 4), "new_Q.shape[0]=" + str(
        new_Q.shape[0]) + " is NOT 4 (invalid Quaternion)!"
    self.X = copy.copy(new_Q)
    self.normalizeQuaternion()
    assert (self.isValid()), (
        "Post-condition(s) checking is failed: this QuaternionDMPState became "
        "invalid!")
    return None
  ...
  def setQd(self, new_Qd):
    assert (self.isValid()), (
        "Pre-condition(s) checking is failed: this QuaternionDMPState is "
        "invalid!")
    assert (new_Qd.shape[0] == 4), "new_Qd.shape[0]=" + str(
        new_Qd.shape[0]) + " is NOT 4!"
  ...
  def setQdd(self, new_Qdd):
    assert (self.isValid()), (
        "Pre-condition(s) checking is failed: this QuaternionDMPState is "
        "invalid!")
    assert (new_Qdd.shape[0] == 4), "new_Qdd.shape[0]=" + str(
        new_Qdd.shape[0]) + " is NOT 4!"
  ...
  def setOmega(self, new_omega):
    assert (self.isValid()), (
        "Pre-condition(s) checking is failed: this QuaternionDMPState is "
        "invalid!")
    assert (new_omega.shape[0] == self.dmp_num_dimensions
           ), "new_omega.shape[0]=" + str(
               new_omega.shape[0]
           ) + " is mis-matched with self.dmp_num_dimensions=" + str(
               self.dmp_num_dimensions) + "!"
  ...
  def setOmegad(self, new_omegad):
    assert (self.isValid()), (
        "Pre-condition(s) checking is failed: this QuaternionDMPState is "
        "invalid!")
    assert (new_omegad.shape[0] == self.dmp_num_dimensions
           ), "new_omegad.shape[0]=" + str(
               new_omegad.shape[0]
           ) + " is mis-matched with self.dmp_num_dimensions=" + str(
               self.dmp_num_dimensions) + "!"
  ...
  def computeQdAndQdd(self):
    ...
    self.normalizeQuaternion()
    [new_QdT, new_QddT
    ] = util_quat.computeQDotAndQDoubleDotTrajectory(self.X.T, self.omega.T,
                                                     self.omegad.T)
