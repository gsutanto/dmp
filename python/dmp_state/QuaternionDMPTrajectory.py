...
class QuaternionDMPTrajectory(QuaternionDMPState, object):
    ...
    def getQuaternionDMPStateAtIndex(self, i):
        ...
        return QuaternionDMPState(self.X[:,i].reshape(4,1), 
                                  self.Xd[:,i].reshape(4,1), 
    ...
    def setQuaternionDMPStateAtIndex(self, i, quatdmpstate):
        ...
        self.omega[:,i] = quatdmpstate.getOmega().reshape(quatdmpstate.dmp_num_dimensions,)
        self.omegad[:,i] = quatdmpstate.getOmegad().reshape(quatdmpstate.dmp_num_dimensions,)
        self.time[:,i] = quatdmpstate.getTime().reshape(1,)
        
        assert (self.isValid()), "Post-condition(s) checking is failed: this QuaternionDMPTrajectory became invalid!"
        return None
    
    def accessQuaternionDMPQAtIndex(self, i): # for quick (pointer-like) access, NO copying
        assert (self.isValid()), "Pre-condition(s) checking is failed: this QuaternionDMPTrajectory is invalid!"
        assert ((i >= 0) and (i < self.time.shape[1])), "Index i=" + str(i) + " is out-of-range (TrajectoryLength=" + str(self.time.shape[1]) + ")!"
        return self.X[:,i].reshape(4,1)
    
    def accessQuaternionDMPQdAtIndex(self, i): # for quick (pointer-like) access, NO copying
        assert (self.isValid()), "Pre-condition(s) checking is failed: this QuaternionDMPTrajectory is invalid!"
        assert ((i >= 0) and (i < self.time.shape[1])), "Index i=" + str(i) + " is out-of-range (TrajectoryLength=" + str(self.time.shape[1]) + ")!"
        return self.Xd[:,i].reshape(4,1)
    
    def accessQuaternionDMPQddAtIndex(self, i): # for quick (pointer-like) access, NO copying
        assert (self.isValid()), "Pre-condition(s) checking is failed: this QuaternionDMPTrajectory is invalid!"
        assert ((i >= 0) and (i < self.time.shape[1])), "Index i=" + str(i) + " is out-of-range (TrajectoryLength=" + str(self.time.shape[1]) + ")!"
        return self.Xdd[:,i].reshape(4,1)
    
    def accessQuaternionDMPOmegaAtIndex(self, i): # for quick (pointer-like) access, NO copying
        assert (self.isValid()), "Pre-condition(s) checking is failed: this QuaternionDMPTrajectory is invalid!"
        assert ((i >= 0) and (i < self.time.shape[1])), "Index i=" + str(i) + " is out-of-range (TrajectoryLength=" + str(self.time.shape[1]) + ")!"
        return self.omega[:,i].reshape(self.dmp_num_dimensions,1)
    
    def accessQuaternionDMPOmegadAtIndex(self, i): # for quick (pointer-like) access, NO copying
        assert (self.isValid()), "Pre-condition(s) checking is failed: this QuaternionDMPTrajectory is invalid!"
        assert ((i >= 0) and (i < self.time.shape[1])), "Index i=" + str(i) + " is out-of-range (TrajectoryLength=" + str(self.time.shape[1]) + ")!"
        return self.omegad[:,i].reshape(self.dmp_num_dimensions,1)
    
    def accessQuaternionDMPTimeAtIndex(self, i): # for quick (pointer-like) access, NO copying
        assert (self.isValid()), "Pre-condition(s) checking is failed: this QuaternionDMPTrajectory is invalid!"
        assert ((i >= 0) and (i < self.time.shape[1])), "Index i=" + str(i) + " is out-of-range (TrajectoryLength=" + str(self.time.shape[1]) + ")!"
        return self.time[:,i].reshape(1,1)

def convertQuaternionDMPStatesListIntoQuaternionDMPTrajectory(quatdmpstates_list):
    ...
    return QuaternionDMPTrajectory(np.concatenate(Q_list,axis=1),
                                   np.concatenate(Qd_list,axis=1),
                                   np.concatenate(Qdd_list,axis=1),
                                   np.concatenate(omega_list,axis=1),
                                   np.concatenate(omegad_list,axis=1),
