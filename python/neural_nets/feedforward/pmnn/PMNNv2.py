...
class PMNNv2(PMNN, object):
    ...
    def performNeuralNetworkPrediction(self, dataset, normalized_phase_kernels, dropout_keep_prob=1.0):
        """
                        ...
                        if (self.is_predicting_only == False):
                            with tf.variable_scope(self.name+'_'+layer_dim_ID, reuse=True):
                                ...
                                if (i < self.N_layers - 2):  # Regular Hidden Layer
                                    affine_intermediate_result = tf.matmul(hidden_drop_dim[dim_out][phase_num], weights) + biases
                            ...
                            if (i < self.N_layers - 2):  # Regular Hidden Layer
                                affine_intermediate_result = np.matmul(hidden_drop_dim[dim_out][phase_num], weights) + biases
                                if (self.neural_net_activation_func_list[i] == 'identity'):
                                    activation_func_output = affine_intermediate_result
                                elif (self.neural_net_activation_func_list[i] == 'tanh'):
                ...
                if (i < self.N_layers - 1): # Hidden Layers (including the Final Hidden Layer with Phase Gating/Modulation)
                    for phase_num in range(self.N_phases):
                        ...
                        if (self.is_predicting_only == False):
                            ...
                            weights = self.model_params[self.name+'_'+layer_dim_ID+"_weights"]
                            biases = self.model_params[self.name+'_'+layer_dim_ID+"_biases"]
                        np.savetxt(dirpath + "/" + dim_out_ID + "/" + phase_ID + "/w" + str(i-1), weights)
                        np.savetxt(dirpath + "/" + dim_out_ID + "/" + phase_ID + "/b" + str(i-1), biases)
                else: # Output Layer
                    layer_dim_ID = layer_name + '_' + dim_out_ID
                    current_layer_dim_size = 1
                    if (self.is_predicting_only == False):
                ...
                if (i < self.N_layers - 1): # Hidden Layers (including the Final Hidden Layer with Phase Gating/Modulation)
                    for phase_num in range(self.N_phases):
                        ...
                        if (i < self.N_layers - 2): # Regular Hidden Layers
                            ...
                            current_layer_dim_size = 1
                        weights_temp = np.loadtxt(dirpath + "/" + dim_out_ID + "/" + phase_ID + "/w" + str(i-1))
                        ...
                        biases_temp = np.loadtxt(dirpath + "/" + dim_out_ID + "/" + phase_ID + "/b" + str(i-1))
                    ...
                    layer_dim_ID = layer_name + '_' + dim_out_ID
                    current_layer_dim_size = 1
                    weights_temp = np.loadtxt(dirpath + "/" + dim_out_ID + "/w" + str(i-1))
                    if (len(weights_temp.shape) == 1):
                        weights_temp = weights_temp.reshape(weights_temp.shape[0], 1)
                    self.model_params[self.name+'_'+layer_dim_ID+"_weights"] = weights_temp
                    if (self.is_predicting_only == False):
