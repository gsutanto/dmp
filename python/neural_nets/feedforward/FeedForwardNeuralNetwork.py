...
class FeedForwardNeuralNetwork(NeuralNetwork):
  ...
  def __init__(self,
               name,
               neural_net_topology,
               nn_hidden_layer_activation_func_list=[],
               filepath=''):
    self.name = name
    ...
    if (nn_hidden_layer_activation_func_list == []):
      ...
      assert (
          len(nn_hidden_layer_activation_func_list) == (self.N_layers - 2)
      ), ('len(nn_hidden_layer_activation_func_list) must be == '
          '(self.N_layers-2)! Only activation functions of the hidden layers '
          'that need to be specified!')
      self.neural_net_activation_func_list = [
          'identity'
      ] + nn_hidden_layer_activation_func_list + ['identity']
    ...
    for i in range(1, self.N_layers):
      ...
      with tf.variable_scope(self.name + '_' + layer_name, reuse=False):
        ...
        biases = tf.get_variable(
            'biases', [self.neural_net_topology[i]],
            initializer=tf.random_normal_initializer(0.0, 1e-14, seed=38))
    ...
    for i in range(1, self.N_layers):
      ...
      with tf.variable_scope(self.name + '_' + layer_name, reuse=True):
        weights = tf.get_variable(
            ...
            [self.neural_net_topology[i - 1], self.neural_net_topology[i]])
        biases = tf.get_variable('biases', [self.neural_net_topology[i]])
        reg_l2_loss = reg_l2_loss + tf.nn.l2_loss(weights) + tf.nn.l2_loss(
            biases)
  ...
  def performNeuralNetworkTraining(self, prediction, ground_truth,
    ...
    pred_l2_loss = tf.nn.l2_loss(
        prediction - ground_truth, name=self.name + '_' + 'my_pred_L2_loss')
    reg_l2_loss = self.computeRegularizationL2Loss()

    loss = tf.reduce_mean(
        pred_l2_loss, name=self.name + '_' + 'my_pred_L2_loss_mean') + (
            beta * reg_l2_loss)

    # Create a variable to track the global step.
    global_step = tf.Variable(
        0, name=self.name + '_' + 'global_step', trainable=False)
  ...
  def loadNeuralNetworkFromMATLABMatFile(self, filepath):
    ...
    for i in range(1, self.N_layers):
      ...
      with tf.variable_scope(self.name + '_' + layer_name, reuse=False):
        weights = tf.get_variable(
            'weights',
            initializer=model_params[self.name + '_' + layer_name + '_weights'])
        weights_dim = weights.get_shape().as_list()
        num_params += weights_dim[0] * weights_dim[1]
        biases = tf.get_variable(
            'biases',
            initializer=model_params[self.name + '_' + layer_name +
                                     '_biases'][0, :])
        num_params += biases.get_shape().as_list()[0]
    print('Total # of Parameters = %d' % num_params)
    return num_params
