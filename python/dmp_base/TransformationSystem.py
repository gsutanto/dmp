...
class TransformationSystem:
    ...
    def __init__(self, canonical_system, function_approximator, 
        ...
        if (current_velocity_dmpstate != None):
            ...
            self.current_velocity_state = DMPState(np.zeros((self.dmp_num_dimensions,1)))
        if (goal_system != None):
    ...
    def isValid(self):
        ...
        assert (len(self.is_using_coupling_term_at_dimension) == self.dmp_num_dimensions), "len(self.is_using_coupling_term_at_dimension)=" + str(len(self.is_using_coupling_term_at_dimension)) + " is mis-matched with self.dmp_num_dimensions=" + str(self.dmp_num_dimensions) + "!"
        ...
        assert (self.current_state.dmp_num_dimensions == self.dmp_num_dimensions), "self.current_state.dmp_num_dimensions=" + str(self.current_state.dmp_num_dimensions) + " is mis-matched with self.dmp_num_dimensions=" + str(self.dmp_num_dimensions) + "!"
    ...
    def getCouplingTerm(self):
        ...
        for transform_coupler_idx in range(len(self.transform_couplers_list)):
            ...
            assert (math.isnan(ct_acc) == False), 'ct_acc['+str(transform_coupler_idx)+'] is NaN!'
            assert (math.isnan(ct_vel) == False), 'ct_vel['+str(transform_coupler_idx)+'] is NaN!'
    ...
    def setCouplingTermUsagePerDimensions(self, is_using_coupling_term_at_dimension_init):
        assert (len(is_using_coupling_term_at_dimension_init) == self.dmp_num_dimensions), "len(is_using_coupling_term_at_dimension_init)=" + str(len(is_using_coupling_term_at_dimension_init)) + " is mis-matched with self.dmp_num_dimensions=" + str(self.dmp_num_dimensions) + "!"
