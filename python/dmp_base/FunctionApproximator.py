...
class FunctionApproximator:
  ...
  def isValid(self):
    ...
    assert (self.canonical_sys !=
            None), "CanonicalSystem canonical_sys does NOT exist!"
    assert (self.canonical_sys.isValid()
           ), "CanonicalSystem canonical_sys is invalid!"
    assert (
        (self.weights.shape[0] == self.dmp_num_dimensions) and
        (self.weights.shape[1] == self.model_size)
    ), "Weights matrix dimensions=" + str(
        self.weights.shape[0]) + "X" + self.weights.shape[
            1] + " is/are mis-matched with self.dmp_num_dimensions=" + str(
                self.dmp_num_dimensions) + " and/or self.model_size=" + str(
                    self.model_size)
    return True
