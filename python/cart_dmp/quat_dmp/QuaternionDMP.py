...
class QuaternionDMP(DMPDiscrete, object):
  ...
  def __init__(self,
               ...
               name=""):
    self.transform_sys_discrete_quat = TransformSystemQuaternion(
        ...
        ts_beta=25.0 / 4.0,
        name="")
    super(QuaternionDMP, self).__init__(
  ...
  def preprocess(self, list_quat_dmp_trajectory):
    assert (self.isValid(
    )), "Pre-condition(s) checking is failed: this QuaternionDMP is invalid!"
    ...
    assert (self.isValid()), (
        "Post-condition(s) checking is failed: this QuaternionDMP became "
        "invalid!")
  ...
  def start(self, critical_states, tau_init):
    ...
    goal_state_init = critical_states.getQuaternionDMPStateAtIndex(
  ...
  def smoothStartEndTrajectoryBasedOnPosition(self, traj, percentage_padding,
    ...
    if (dt is None):
      ...
      assert (dt > 0.0)
    return smoothStartEndQuatTrajectoryBasedOnQuaternion(
        Quat_traj=traj,
  ...
  def learnGetDefaultUnrollParams(self, set_traj_input, robot_task_servo_rate):
    W, mean_A_learn, mean_tau, Ft, Fp, QgT, cX, cV, PSI = self.learn(
    ...
    critical_states_learn = convertQuaternionDMPStatesListIntoQuaternionDMPTrajectory(
  ...
  def convertDMPStatesListIntoDMPTrajectory(self, dmpstates_list):
    return convertQuaternionDMPStatesListIntoQuaternionDMPTrajectory(
        dmpstates_list)
  ...
  def plotDemosVsUnroll(self,
    ...
    return py_util.plotManyTrajsVsOneTraj(
        set_many_trajs=set_demo_Qtrajs,
        one_traj=unroll_Qtraj,
