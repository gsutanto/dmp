...
class CartesianCoordDMPTensorflow(CartesianCoordDMP, object):
  """
    Class for discrete Cartesian Coordinate (x-y-z) DMPs, implemented with
    TensorFlow.
    Only valid for 2nd-order canonical system.
    Terminology:
    z_vec             = augmented_state_vector               (size
    ((3*dmp_num_dimensions)+1+1)X1)
    x_vec             = state_position_vector                (size
    dmp_num_dimensionsX1)
    v_vec             = state_velocity_vector**              (size
    dmp_num_dimensionsX1)
    g_vec             = goal_state_position_vector           (size
    dmp_num_dimensionsX1)
    p_vec             = phase_variable_vector                (size
    1X1)
    u_vec             = phase_velocity_vector                (size
    1X1)
    G_vec             = steady_state_goal_position_vector    (size
    dmp_num_dimensionsX1)
    ft                = forcing_term
    ft_kernel_mu_vec  = forcing_term_kernel_center_vector    (size
    model_sizeX1)
    ft_kernel_chi_vec = forcing_term_kernel_bandwidth_vector (size
    model_sizeX1)
    ft_W_mat          = forcing_term_weight_matrix           (size
    dmp_num_dimensionsXmodel_size)
    ct                = coupling_term
  ...
  def TFExtractStateComponents(self, tf_z_vec):
    tf.assert_equal(
        tf.shape(tf_z_vec)[0], ((3 * self.dmp_num_dimensions) + 1 + 1))
    tf.assert_equal(tf.shape(tf_z_vec)[1], 1)

    tf_x_vec = tf.slice(tf_z_vec, [0 * self.dmp_num_dimensions, 0],
                        [self.dmp_num_dimensions, 1])
    tf_v_vec = tf.slice(tf_z_vec, [1 * self.dmp_num_dimensions, 0],
                        [self.dmp_num_dimensions, 1])
    tf_g_vec = tf.slice(tf_z_vec, [2 * self.dmp_num_dimensions, 0],
                        [self.dmp_num_dimensions, 1])
    tf_p_vec = tf.slice(tf_z_vec, [(3 * self.dmp_num_dimensions) + 0, 0],
                        [1, 1])
    tf_u_vec = tf.slice(tf_z_vec, [(3 * self.dmp_num_dimensions) + 1, 0],
                        [1, 1])

    return [tf_x_vec, tf_v_vec, tf_g_vec, tf_p_vec, tf_u_vec]

  def TFAugmentStateComponents(self, tf_x_vec, tf_v_vec, tf_g_vec, tf_p_vec,
                               tf_u_vec):
    ...
    tf_z_vec = tf.concat([tf_x_vec, tf_v_vec, tf_g_vec, tf_p_vec, tf_u_vec],
                         axis=0)
  ...
  def TFComputeNextAugmentedState(self,
    ...
    [tf_curr_x_vec, tf_curr_v_vec, tf_curr_g_vec, tf_curr_p_vec,
     tf_curr_u_vec] = self.TFExtractStateComponents(tf_curr_z_vec)
    ...
    tf_ft_kernel_vec = tf.exp(-0.5 * tf.square(
        tf.matmul(tf.ones([self.model_size, 1], tf.float32), tf_curr_p_vec) -
        tf_ft_kernel_mu_vec) * tf_ft_kernel_chi_vec)
    tf_normalized_ft_kernel_matmul_u_vec = tf.matmul(
        (tf_ft_kernel_vec /
         tf.reduce_sum(tf_ft_kernel_vec + 1.e-10, axis=0)[0]), tf_curr_u_vec)
    tf_ft_vec = tf.matmul(tf_ft_W_mat, tf_normalized_ft_kernel_matmul_u_vec)
    tf_fdync_curr_v_vec = (((tf_alpha_v_sca /
                             (tf_tau_relative_sca * tf_tau_relative_sca)) *
    ...
    tf_fdync_curr_g_vec = ((tf_alpha_g_sca / tf_tau_relative_sca) *
                           (tf_G_vec - tf_curr_g_vec))
    ...
    tf_fdync_curr_z_vec = tf.concat([
        tf_fdync_curr_x_vec, tf_fdync_curr_v_vec, tf_fdync_curr_g_vec,
        tf_fdync_curr_p_vec, tf_fdync_curr_u_vec
    ],
                                    axis=0)

    tf_u_curr_x_vec = tf.zeros([self.dmp_num_dimensions, 1], tf.float32)
    tf_u_curr_v_vec = ((1.0 / (tf_tau_relative_sca * tf_tau_relative_sca)) *
                       tf_ct_vec)
    ...
    tf_u_curr_z_vec = tf.concat([
        tf_u_curr_x_vec, tf_u_curr_v_vec, tf_u_curr_g_vec, tf_u_curr_p_vec,
        tf_u_curr_u_vec
    ],
                                axis=0)
    ...
    tf.assert_equal(
        tf.shape(tf_next_z_vec)[0], ((3 * self.dmp_num_dimensions) + 1 + 1))
    tf.assert_equal(tf.shape(tf_next_z_vec)[1], 1)

    tf.assert_equal(
        tf.shape(tf_fdynd_curr_z_vec)[0],
        ((3 * self.dmp_num_dimensions) + 1 + 1))
    tf.assert_equal(tf.shape(tf_fdynd_curr_z_vec)[1], 1)

    tf.assert_equal(
        tf.shape(tf_ud_curr_z_vec)[0], ((3 * self.dmp_num_dimensions) + 1 + 1))
  ...
  def GetTFGraphComputeNextAugmentedState(self, is_returning_as_graph_def=True):
    ...
    with g.as_default() as g:
      tf_curr_z_vec_ph = tf.placeholder(
          tf.float32,
          shape=[((3 * self.dmp_num_dimensions) + 1 + 1), 1],
          name="tf_curr_z_vec_placeholder")
      tf_G_vec_ph = tf.placeholder(
          tf.float32,
          shape=[self.dmp_num_dimensions, 1],
          name="tf_G_vec_placeholder")
      tf_ft_kernel_mu_vec_ph = tf.placeholder(
          tf.float32,
          shape=[self.model_size, 1],
          name="tf_ft_kernel_mu_vec_placeholder")
      tf_ft_kernel_chi_vec_ph = tf.placeholder(
          tf.float32,
          shape=[self.model_size, 1],
          name="tf_ft_kernel_chi_vec_placeholder")
      tf_ft_W_mat_ph = tf.placeholder(
          tf.float32,
          shape=[self.dmp_num_dimensions, self.model_size],
          name="tf_ft_W_mat_placeholder")
      tf_ft_scaler_vec_ph = tf.placeholder(
          tf.float32,
          shape=[self.dmp_num_dimensions, 1],
          name="tf_ft_scaler_vec_placeholder")
      tf_ct_vec_ph = tf.placeholder(
          tf.float32,
          shape=[self.dmp_num_dimensions, 1],
          name="tf_ct_vec_placeholder")
      tf_tau_relative_sca_ph = tf.placeholder(
          tf.float32, shape=[], name="tf_tau_relative_sca_placeholder")
      tf_dt_sca_ph = tf.placeholder(
          tf.float32, shape=[], name="tf_dt_sca_placeholder")
      tf_alpha_v_sca_ph = tf.placeholder_with_default(
          25.0, shape=[], name="tf_alpha_v_sca_placeholder")
      tf_beta_v_sca_ph = tf.placeholder_with_default(
          25.0 / 4.0, shape=[], name="tf_beta_v_sca_placeholder")
      tf_alpha_g_sca_ph = tf.placeholder_with_default(
          25.0 / 2.0, shape=[], name="tf_alpha_g_sca_placeholder")
      tf_alpha_u_sca_ph = tf.placeholder_with_default(
          25.0, shape=[], name="tf_alpha_u_sca_placeholder")
      tf_beta_u_sca_ph = tf.placeholder_with_default(
          25.0 / 4.0, shape=[], name="tf_beta_u_sca_placeholder")

      [tf_next_z_vec, tf_fdynd_curr_z_vec,
       tf_ud_curr_z_vec] = self.TFComputeNextAugmentedState(
           tf_curr_z_vec_ph, tf_G_vec_ph, tf_ft_kernel_mu_vec_ph,
           tf_ft_kernel_chi_vec_ph, tf_ft_W_mat_ph, tf_ft_scaler_vec_ph,
           tf_ct_vec_ph, tf_tau_relative_sca_ph, tf_dt_sca_ph,
           tf_alpha_v_sca_ph, tf_beta_v_sca_ph, tf_alpha_g_sca_ph,
           tf_alpha_u_sca_ph, tf_beta_u_sca_ph)

      tf_next_z_vec_ph = tf.identity(
          tf_next_z_vec, name="tf_next_z_vec_placeholder")  # output placeholder
      tf_fdynd_curr_z_vec_ph = tf.identity(
          tf_fdynd_curr_z_vec,
          name="tf_fdynd_curr_z_vec_placeholder")  # output placeholder
      tf_ud_curr_z_vec_ph = tf.identity(
          tf_ud_curr_z_vec,
          name="tf_ud_curr_z_vec_placeholder")  # output placeholder
  ...
  def getNextState(self,
    ...
    assert (self.transform_sys_discrete_cart_coord.isValid()), (
        "Pre-condition(s) checking is failed: the TransformSystemDiscrete of "
        "this CartesianCoordDMP is invalid!")
    assert (dt > 0.0)
    ...
    for d in range(self.dmp_num_dimensions):
      if (self.transform_sys_discrete_cart_coord.is_using_scaling[d]):
        if (np.fabs(self.transform_sys_discrete_cart_coord.A_learn[d, 0]) <
            MIN_FABS_AMPLITUDE):
          ft_scaler[d, 0] = 1.0
        else:
          ft_scaler[d, 0] = ft_scaler[
              d, 0] * 1.0 / self.transform_sys_discrete_cart_coord.A_learn[d, 0]
      else:
        ft_scaler[d, 0] = 1.0
