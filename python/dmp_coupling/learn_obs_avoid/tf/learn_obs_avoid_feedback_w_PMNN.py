...
#input_selector = 1 # X_raw input, Phase Radial Basis Function Network (PRBFN) with  2 regular hidden layers of 100 (relu) and 50 (tanh) nodes, and 25 nodes in the phase-modulated final hidden layer (regular execution)
...
for input_selector in input_selector_list:
    ...
    if (input_selector == 1):
        input_X_descriptor_string = 'raw_reg_hidden_layer_100relu_50tanh'
    ...
    while os.path.exists(file_check_path):
        ...
        for prim_no in range(1, 2):
            ...
            if (input_selector == 1):
                regular_NN_hidden_layer_topology = [100, 50]
