...
for input_selector in input_selector_list:
    ...
    while os.path.exists(file_check_path):
        ...
        for prim_no in range(1, 2):
            ...
            # load dataset:
            if (input_selector == 1):
                X = sio.loadmat('input_data/'+generalization_test_sub_path+'prim_'+str(prim_no)+'_X_raw_obs_avoid_diff_Ct_dataset'+generalization_test_id_string+'.mat', struct_as_record=True)['X'].astype(np.float32)
                X_generalization_test = sio.loadmat('input_data/'+generalization_test_sub_path+'test_unroll_prim_'+str(prim_no)+'_X_raw_obs_avoid_diff_Ct_dataset'+generalization_test_id_string+'.mat', struct_as_record=True)['X'].astype(np.float32)
            
            diff_Ct_target = sio.loadmat('input_data/'+generalization_test_sub_path+'prim_'+str(prim_no)+'_diff_Ct_target_obs_avoid_diff_Ct_dataset'+generalization_test_id_string+'.mat', struct_as_record=True)['diff_Ct_target'].astype(np.float32)
            normalized_phase_kernels_times_dt_per_tau = sio.loadmat('input_data/'+generalization_test_sub_path+'prim_'+str(prim_no)+'_normalized_phase_PSI_mult_phase_V_times_dt_per_tau_obs_avoid_diff_Ct_dataset'+generalization_test_id_string+'.mat', struct_as_record=True)['normalized_phase_PSI_mult_phase_V_times_dt_per_tau'].astype(np.float32)
            data_point_priority = sio.loadmat('input_data/'+generalization_test_sub_path+'prim_'+str(prim_no)+'_data_point_priority_obs_avoid_diff_Ct_dataset'+generalization_test_id_string+'.mat', struct_as_record=True)['data_point_priority'].astype(np.float32)
            Ct_t_minus_1_times_dt_per_tau = sio.loadmat('input_data/'+generalization_test_sub_path+'prim_'+str(prim_no)+'_Ct_t_minus_1_times_dt_per_tau_obs_avoid_diff_Ct_dataset'+generalization_test_id_string+'.mat', struct_as_record=True)['Ct_t_minus_1_times_dt_per_tau'].astype(np.float32)
            ...
            diff_Ctt_generalization_test = sio.loadmat('input_data/'+generalization_test_sub_path+'test_unroll_prim_'+str(prim_no)+'_diff_Ct_target_obs_avoid_diff_Ct_dataset'+generalization_test_id_string+'.mat', struct_as_record=True)['diff_Ct_target'].astype(np.float32)
            nPSI_times_dt_per_tau_generalization_test = sio.loadmat('input_data/'+generalization_test_sub_path+'test_unroll_prim_'+str(prim_no)+'_normalized_phase_PSI_mult_phase_V_times_dt_per_tau_obs_avoid_diff_Ct_dataset'+generalization_test_id_string+'.mat', struct_as_record=True)['normalized_phase_PSI_mult_phase_V_times_dt_per_tau'].astype(np.float32)
            W_generalization_test = sio.loadmat('input_data/'+generalization_test_sub_path+'test_unroll_prim_'+str(prim_no)+'_data_point_priority_obs_avoid_diff_Ct_dataset'+generalization_test_id_string+'.mat', struct_as_record=True)['data_point_priority'].astype(np.float32)
            Ct_t_minus_1_times_dt_per_tau_generalization_test = sio.loadmat('input_data/'+generalization_test_sub_path+'test_unroll_prim_'+str(prim_no)+'_Ct_t_minus_1_times_dt_per_tau_obs_avoid_diff_Ct_dataset'+generalization_test_id_string+'.mat', struct_as_record=True)['Ct_t_minus_1_times_dt_per_tau'].astype(np.float32)
            
            print('X_generalization_test.shape                             =', X_generalization_test.shape)
            print('diff_Ctt_generalization_test.shape                      =', diff_Ctt_generalization_test.shape)
            ...
            N_data = diff_Ct_target.shape[0]
            D_input = X.shape[1]
            D_output = diff_Ct_target.shape[1]
            print('N_data   =', N_data)
            ...
            diff_Ctt_valid = diff_Ct_target[idx_valid_dataset,:]
            ...
            diff_Ctt_test = diff_Ct_target[idx_test_dataset,:]
            ...
            with ff_nn_graph.as_default():
                ...
                train_batch_prediction = rpmnn.performNeuralNetworkPrediction(tf_train_X_batch, tf_train_nPSI_times_dt_per_tau_batch, tf_train_Ct_t_minus_1_times_dt_per_tau_batch, tf_train_dropout_keep_prob)
                ...
                train_prediction = rpmnn.performNeuralNetworkPrediction(tf_train_X, tf_train_nPSI_times_dt_per_tau, tf_train_Ct_t_minus_1_times_dt_per_tau, 1.0)
                valid_prediction = rpmnn.performNeuralNetworkPrediction(tf_valid_X, tf_valid_nPSI_times_dt_per_tau, tf_valid_Ct_t_minus_1_times_dt_per_tau, 1.0)
                test_prediction  = rpmnn.performNeuralNetworkPrediction(tf_test_X, tf_test_nPSI_times_dt_per_tau, tf_test_Ct_t_minus_1_times_dt_per_tau, 1.0)
                generalization_test_prediction  = rpmnn.performNeuralNetworkPrediction(tf_generalization_test_X, tf_generalization_test_nPSI_times_dt_per_tau, tf_generalization_test_Ct_t_minus_1_times_dt_per_tau, 1.0)
        ...
        #                sys.exit("ERROR: rpmnn.num_params=%d >= %d=N_train_dataset" % (rpmnn.num_params, N_train_dataset))
                    ...
                    for step in range(TF_max_train_iters):
                        ...
                        offset = (step * batch_size) % (diff_Ctt_train.shape[0] - batch_size)
                        ...
                        batch_diff_Ctt = diff_Ctt_train[offset:(offset + batch_size), :]
            ...
            #            _, loss_value, tr_batch_prediction, summary = session.run([train_op, loss, train_batch_prediction, summary_op], feed_dict=feed_dict)
                        ...
                        if (is_performing_generalization_test == 0):
                            if (step % 1000 == 0):
                                ...
                                print("Minibatch NMSE :", computeNMSE(tr_batch_prediction, batch_diff_Ctt))
                            if ((step % 5000 == 0) or ((step > 0) and ((step == np.power(10,(np.floor(np.log10(step)))).astype(np.int32)) or (step == 5 * np.power(10,(np.floor(np.log10(step/5)))).astype(np.int32))))):
                                ...
                                if ((is_performing_weighted_training) and (step % 5000 == 0) and (step > 0)):
                                    wnmse_train = computeWNMSE(train_prediction.eval(), diff_Ctt_train, W_train)
                                    wnmse_valid = computeWNMSE(valid_prediction.eval(), diff_Ctt_valid, W_valid)
                                    wnmse_test = computeWNMSE(test_prediction.eval(), diff_Ctt_test, W_test)
                                    wnmse_generalization_test = computeWNMSE(generalization_test_prediction.eval(), diff_Ctt_generalization_test, W_generalization_test)
        ...
        #                            print("Generalization Test WNMSE: ", wnmse_generalization_test)
                                ...
                                nmse_train = computeNMSE(train_prediction.eval(), diff_Ctt_train)
                                nmse_valid = computeNMSE(valid_prediction.eval(), diff_Ctt_valid)
                                nmse_test = computeNMSE(test_prediction.eval(), diff_Ctt_test)
                                nmse_generalization_test = computeNMSE(generalization_test_prediction.eval(), diff_Ctt_generalization_test)
            ...
            #                    if ((step > 0) and ((step == np.power(10,(np.floor(np.log10(step)))).astype(np.int32)) or (step == 5 * np.power(10,(np.floor(np.log10(step/5)))).astype(np.int32)))):
                                ...
                                sio.savemat((model_output_dir_path+'diff_Ct_dataset_prim_'+str(prim_no)+'_params_reinit_'+str(n_NN_reinit_trial)+'_step_%07d'%step+'.mat'), NN_model_params)
                                ...
                                sio.savemat((model_output_dir_path+'diff_Ct_dataset_prim_'+str(prim_no)+'_nmse_reinit_'+str(n_NN_reinit_trial)+'_step_%07d'%step+'.mat'), nmse)
                                var_ground_truth = {}
                                var_ground_truth["var_ground_truth_diff_Ctt_train"] = var_ground_truth_diff_Ctt_train
                                sio.savemat((model_output_dir_path+'diff_Ct_dataset_prim_'+str(prim_no)+'_var_ground_truth.mat'), var_ground_truth)
                        elif (is_performing_generalization_test == 1):
                            if ((n_NN_reinit_trial == 0) and (step == 0)):
                                best_nmse_generalization_test = computeNMSE(generalization_test_prediction.eval(), diff_Ctt_generalization_test)
                                best_nmse_train = computeNMSE(train_prediction.eval(), diff_Ctt_train)
                                best_nmse_valid = computeNMSE(valid_prediction.eval(), diff_Ctt_valid)
                                best_nmse_test = computeNMSE(test_prediction.eval(), diff_Ctt_test)
                            elif (step % 500 == 0):
                                nmse_generalization_test = computeNMSE(generalization_test_prediction.eval(), diff_Ctt_generalization_test)
    ...
    #                        print("Final Generalization Test WNMSE: ", wnmse_generalization_test)
                        ...
                        sio.savemat((model_output_dir_path+'diff_Ct_dataset_prim_'+str(prim_no)+'_best_nmse_trial_'+str(generalization_test_id)+'.mat'), best_nmse)
        if (is_performing_generalization_test == 1):
