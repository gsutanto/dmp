...
logs_path = "/tmp/pmnnv2/"

NN_name = 'my_PMNNv2_obs_avoid_fb'
...
#input_selector = 1 # X_raw input, Phase-Modulated Neural Network (PMNNv2) with  2 regular hidden layers of 100 (relu), and 75 (tanh) nodes, and 25 nodes in the phase-modulated final hidden layer (regular execution)
...
#        input_X_descriptor_string = 'raw_reg_hidden_layer_100relu_75tanh'
    ...
    while os.path.exists(file_check_path):
        ...
        for prim_no in range(1, 2):
            ...
            pmnnv2_graph = tf.Graph()
            with pmnnv2_graph.as_default():
                ...
                train_batch_prediction = pmnnv2.performNeuralNetworkPrediction(tf_train_X_batch, tf_train_nPSI_batch, tf_train_dropout_keep_prob)
                
                # Build the Training Graph (that calculate and apply gradients), per output dimension.
                if (is_performing_weighted_training):
                    train_op_dim0, loss_dim0 = pmnnv2.performNeuralNetworkWeightedTrainingPerDimOut(train_batch_prediction, tf_train_Ctt_batch, init_learning_rate, beta, 0, tf_train_W_batch)
                    train_op_dim1, loss_dim1 = pmnnv2.performNeuralNetworkWeightedTrainingPerDimOut(train_batch_prediction, tf_train_Ctt_batch, init_learning_rate, beta, 1, tf_train_W_batch)
                    train_op_dim2, loss_dim2 = pmnnv2.performNeuralNetworkWeightedTrainingPerDimOut(train_batch_prediction, tf_train_Ctt_batch, init_learning_rate, beta, 2, tf_train_W_batch)
                else:
                    train_op_dim0, loss_dim0 = pmnnv2.performNeuralNetworkTrainingPerDimOut(train_batch_prediction, tf_train_Ctt_batch, init_learning_rate, beta, 0)
                    train_op_dim1, loss_dim1 = pmnnv2.performNeuralNetworkTrainingPerDimOut(train_batch_prediction, tf_train_Ctt_batch, init_learning_rate, beta, 1)
                    train_op_dim2, loss_dim2 = pmnnv2.performNeuralNetworkTrainingPerDimOut(train_batch_prediction, tf_train_Ctt_batch, init_learning_rate, beta, 2)
                ...
                train_prediction = pmnnv2.performNeuralNetworkPrediction(tf_train_X, tf_train_nPSI, 1.0)
                valid_prediction = pmnnv2.performNeuralNetworkPrediction(tf_valid_X, tf_valid_nPSI, 1.0)
                test_prediction  = pmnnv2.performNeuralNetworkPrediction(tf_test_X, tf_test_nPSI, 1.0)
                generalization_test_prediction  = pmnnv2.performNeuralNetworkPrediction(tf_generalization_test_X, tf_generalization_test_nPSI, 1.0)
            ...
            with tf.Session(graph=pmnnv2_graph) as session:
                for n_NN_reinit_trial in range(N_NN_reinit_trials):
                    ...
                    if (pmnnv2.num_params < N_train_dataset):
                        print("OK: pmnnv2.num_params=%d < %d=N_train_dataset" % (pmnnv2.num_params, N_train_dataset))
                    else:
                        print(Fore.RED + "WARNING: pmnnv2.num_params=%d >= %d=N_train_dataset" % (pmnnv2.num_params, N_train_dataset))
                        print(Style.RESET_ALL)
        #                sys.exit("ERROR: pmnnv2.num_params=%d >= %d=N_train_dataset" % (pmnnv2.num_params, N_train_dataset))
        ...
        #                            print("Generalization Test WNMSE: ", wnmse_generalization_test)
            ...
            #                    if ((step > 0) and ((step == np.power(10,(np.floor(np.log10(step)))).astype(np.int32)) or (step == 5 * np.power(10,(np.floor(np.log10(step/5)))).astype(np.int32)))):
                                NN_model_params = pmnnv2.saveNeuralNetworkToMATLABMatFile()
