...
init_model_param_filepath = model_parent_dir_path + 'recur_Ct_dataset_prim_' + str(prim_no+1) + '_params_reinit_' + str(reinit_selection_idx[prim_no]) + ('_step_%07d.mat' % TF_max_train_iters)
...
#        print('X_train.shape                                   =', X_train.shape)
        ...
        batch_Ct_t_minus_1 = Ct_t_minus_1_train
        batch_Ctt = Ctt_train
...
#             if ((step > 0) and ((step == np.power(10,(np.floor(np.log10(step)))).astype(np.int32)) or (step == 5 * np.power(10,(np.floor(np.log10(step/5)))).astype(np.int32)))):
            sio.savemat((model_output_dir_path+'recur_Ct_dataset_prim_'+str(prim_no+1)+'_params_step_%07d'%step+'.mat'), NN_model_params)
            nmse["nmse_train"] = nmse_train
            sio.savemat((model_output_dir_path+'recur_Ct_dataset_prim_'+str(prim_no+1)+'_nmse_step_%07d'%step+'.mat'), nmse)
            var_ground_truth = {}
            var_ground_truth["var_ground_truth_Ctt_train"] = var_ground_truth_Ctt_train
            sio.savemat((model_output_dir_path+'recur_Ct_dataset_prim_'+str(prim_no+1)+'_var_ground_truth.mat'), var_ground_truth)
        if (step % 50 == 0):
            np.savetxt((model_output_dir_path+'recur_Ct_dataset_batch_nmse_train_log.txt'), batch_nmse_train_log[0:step,:])
    print("")
#     if (is_performing_weighted_training):
