...
initial_model_parent_dir_path = '../models/'

# Create directories if not currently exist:
rl_model_parent_dir_path = './models/'
if not os.path.isdir(rl_model_parent_dir_path):
    os.makedirs(rl_model_parent_dir_path)

N_NN_reinit_trials = 3
batch_size = 64
TF_max_train_iters = np.loadtxt(initial_model_parent_dir_path+'TF_max_train_iters.txt', dtype=np.int, ndmin=0) + 1
...
# X_raw input, Phase-Modulated Neural Network (PMNN) with 1 regular hidden layer only of 100 nodes and 25 nodes in the phase-modulated final hidden layer (regular execution)

# Initial Learning Rate
init_learning_rate = 0.001

rl_model_output_dir_path = './models/'
if not os.path.isdir(rl_model_output_dir_path):
    os.makedirs(rl_model_output_dir_path)
...
for prim_no in range(2, 4):
    ...
    Ct_target = sio.loadmat('scraping/prim_'+str(prim_no)+'_Ct_target_scraping.mat', struct_as_record=True)['Ct_target'].astype(np.float32)
    normalized_phase_kernels = sio.loadmat('scraping/prim_'+str(prim_no)+'_normalized_phase_PSI_mult_phase_V_scraping.mat', struct_as_record=True)['normalized_phase_PSI_mult_phase_V'].astype(np.float32)
    data_point_priority = sio.loadmat('scraping/prim_'+str(prim_no)+'_data_point_priority_scraping.mat', struct_as_record=True)['data_point_priority'].astype(np.float32)
    ...
    X_generalization_test = sio.loadmat('scraping/test_unroll_prim_'+str(prim_no)+'_X_raw_scraping.mat', struct_as_record=True)['X'].astype(np.float32)
    Ctt_generalization_test = sio.loadmat('scraping/test_unroll_prim_'+str(prim_no)+'_Ct_target_scraping.mat', struct_as_record=True)['Ct_target'].astype(np.float32)
    nPSI_generalization_test = sio.loadmat('scraping/test_unroll_prim_'+str(prim_no)+'_normalized_phase_PSI_mult_phase_V_scraping.mat', struct_as_record=True)['normalized_phase_PSI_mult_phase_V'].astype(np.float32)
    W_generalization_test = sio.loadmat('scraping/test_unroll_prim_'+str(prim_no)+'_data_point_priority_scraping.mat', struct_as_record=True)['data_point_priority'].astype(np.float32)
    ...
    regular_NN_hidden_layer_topology = list(np.loadtxt(initial_model_parent_dir_path+'regular_NN_hidden_layer_topology.txt', dtype=np.int, ndmin=1))
    ...
    regular_NN_hidden_layer_activation_func_list = list(np.loadtxt(initial_model_parent_dir_path+'regular_NN_hidden_layer_activation_func_list.txt', dtype=np.str, ndmin=1)) * len(regular_NN_hidden_layer_topology)
...
#                    if ((step > 0) and ((step == np.power(10,(np.floor(np.log10(step)))).astype(np.int32)) or (step == 5 * np.power(10,(np.floor(np.log10(step/5)))).astype(np.int32)))):
                    ...
                    sio.savemat((rl_model_output_dir_path+'prim_'+str(prim_no)+'_params_reinit_'+str(n_NN_reinit_trial)+'_step_%07d'%step+'.mat'), NN_model_params)
                    ...
                    sio.savemat((rl_model_output_dir_path+'prim_'+str(prim_no)+'_nmse_reinit_'+str(n_NN_reinit_trial)+'_step_%07d'%step+'.mat'), nmse)
                    var_ground_truth = {}
                    var_ground_truth["var_ground_truth_Ctt_train"] = var_ground_truth_Ctt_train
                    sio.savemat((rl_model_output_dir_path+'prim_'+str(prim_no)+'_var_ground_truth.mat'), var_ground_truth)
            print("")
#            if (is_performing_weighted_training):
