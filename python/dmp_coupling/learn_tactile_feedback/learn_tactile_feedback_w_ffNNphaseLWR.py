...
#input_selector = 3 # X_raw input,                                      PRBFN  with  1 regular hidden layer only of 100 nodes and  8 nodes in the phase-modulated final hidden layer (comparison: between different number of nodes in the regular hidden layer)
...
#                X_generalization_test = sio.loadmat('scraping/'+generalization_test_sub_path+'test_unroll_prim_'+str(prim_no)+'_X_dim_reduced_autoencoder_scraping'+generalization_test_id_string+'.mat', struct_as_record=True)['X_dim_reduced_autoencoder'].astype(np.float32)
                ...
                with ae_graph.as_default():
                    ...
                    AE = AutoEncoder(ae_name, X_raw.shape[1], encoder_NN_hidden_layer_topology, D_latent, ae_model_filepath)
            ...
            with ff_nn_graph.as_default():
                ...
                ffNNphaseLWR = FFNNFinalPhaseLWRLayerPerDims(NN_name, D_input, regular_NN_hidden_layer_topology, N_phaseLWR_kernels, D_output, "", is_using_phase_kernel_modulation)
