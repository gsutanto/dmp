  ...
  def __init__(self,
    ...
    if (tau_init == None):
      if (robot_task_servo_rate == None):
        self.tau = goal_dmpstate.getTime()[0, 0] - start_dmpstate.getTime()[0,
                                                                            0]
      else:
      ...
      self.tau = tau_init
    assert (self.tau >=
            MIN_TAU), "DMPUnrollInitParams.tau=" + str(self.tau) + " < MIN_TAU"
    critical_dmpstates_list = []
    critical_dmpstates_list.append(start_dmpstate)
    if (traj_size >= 3):
      before_goal_dmpstate = critical_dmptraj_init.getDMPStateAtIndex(
          traj_size - 2)
    ...
    for critical_dmpstate in critical_dmpstates_list:
      if (is_zeroing_out_velocity_and_acceleration):
        processed_critical_states_list.append(
            DMPState(critical_dmpstate.getX()))
      else:
        processed_critical_states_list.append(copy.copy(critical_dmpstate))
    self.critical_states = convertDMPStatesListIntoDMPTrajectory(
        processed_critical_states_list)
...
def getDMPUnrollInitParams(start_dmpstate,
                           goal_dmpstate,
                           tau_init=None,
                           robot_task_servo_rate=None,
                           is_zeroing_out_velocity_and_acceleration=True):
  critical_dmptraj_init = convertDMPStatesListIntoDMPTrajectory(
      [start_dmpstate, goal_dmpstate])
  return DMPUnrollInitParams(critical_dmptraj_init, tau_init,
