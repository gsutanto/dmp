...
% necessary:
in_data_dir_path    = ['~/Desktop/dmp_demos/',task_type,'/'];
...
for iter_obj = contents'
    ...
    if ((strcmp(iter_obj.name, '.') == 0) && (strcmp(iter_obj.name, '..') == 0)...
        && (strcmp(iter_obj.name, 'old') == 0) && (isdir(in_iter_obj_path)))
...
%             end
            ...
            % baseline setting:
            if ((data_file_count == 1) && (strcmp(iter_obj.name, 'baseline') == 1))
%             if (data_file_count == 1)
            ...
            for np_idx = 1:N_primitive
                ...
                if (((data_file_count == 1) && (strcmp(iter_obj.name, 'baseline') == 1)) || ...
                    (~((data_file_count == 1) && (strcmp(iter_obj.name, 'baseline') == 1)) && ((np == 1) || (np == 3))))
...
%                         if (is_visualizing_traj_alignment && (np == 1))
                            ...
                            for d=1:N_dim_alignment_traj
                                ...
                                hold on;
                                    title(['Alignment ',iter_obj.name,'/',data_file.name,' Dimension #', num2str(dim_alignment_traj_cell{np,1}(d))]);
...
%                                     plot(ref_traj_cell(:,d), 'r');
                            ...
                            for d=1:N_dim_alignment_traj
                                subplot(N_dim_alignment_traj, 1, d);
                                hold on;
                                    title(['Unclipped Trajectory ',iter_obj.name,'/',data_file.name,' Dimension #', num2str(dim_alignment_traj_cell{np,1}(d))]);
                            ...
                            for np_eval = [1,3]
                                ...
                                if (is_plotting_excluded_demo_segmentation)
                                    ...
                                    for d=1:N_dim_alignment_traj
                                        subplot(N_dim_alignment_traj, 1, d);
                                        hold on;
                                            title(['Alignment ',iter_obj.name,'/',data_file.name,' Dimension #', num2str(dim_alignment_traj_cell{np_eval,1}(d)), ', Primitive #', num2str(np_eval)]);
                                    ...
                                    figure_exclude_save_path  = [in_iter_obj_path,'/exclude/',data_file.name,'_prim',num2str(np_eval,'%02d'),'.jpg'];
                                ...
                                prim_to_be_deleted_file_path = [out_iter_obj_path,'/prim',...
                                                                num2str(np_eval,'%02d'),'/',...
...
%                 axis equal;
%                 hold on;
%                     pyz                 = plot(1500*sm_traj(:,3:4), 'g');                   % R_HAND y-z
%                     pydzd               = plot(10000*sm_traj(:,6:7), 'c');                  % R_HAND yd-zd
%                     filt_ydd_zdd        = filtfilt(b, a, sm_traj(:,9:10));
%                     pyddzdd             = plot(6000*[zeros(200,2);filt_ydd_zdd(200:end,:)], 'b');   % R_HAND ydd-zdd
%                     pR_LF_forces        = plot(700*sm_traj(:,71:73), 'm');                  % R_LF forces
%                     pR_LF_electrodes    = plot(sm_traj(:,27:45), 'r');                      % R_LF electrodes
%                     plot(R_electrodes_based_clip_retain_idx_cell{1,np}(1), 0, 'ko',...
%                          'LineWidth',3, 'MarkerSize', 10,...
%                          'MarkerFaceColor','k');
%                     plot(retain_idx_cell{1,np}(end), 0, 'ko',...
%                          'LineWidth',3, 'MarkerSize', 10,...
%                          'MarkerFaceColor','k');
%                     legend([pyz(1), pydzd(1), pyddzdd(1), pR_LF_forces(1), pR_LF_electrodes(1)],...
%                            'y-z', 'yd-zd', 'ydd-zdd', 'R\_LF', 'R\_electrode');
%                 hold off;
%                 keyboard;
