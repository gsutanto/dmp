 ...
 * NON-REAL-TIME!!!
 */
DMPUnrollInitParams::DMPUnrollInitParams(const double& tau_init,
    ...
    tau(tau_init), rt_assertor(real_time_assertor)
{
    ...
    if (traj_size >= 2)
    {
        ...
        if (critical_states.size() >= 2)
        {
            if (is_zeroing_out_velocity_and_acceleration)
            {
                critical_states[critical_states.size()-1]   = DMPState(trajectory[traj_size-1].getX(), rt_assertor);
            }
            else
            {
        ...
        if (critical_states.size() >= 3)
        {
            if (is_zeroing_out_velocity_and_acceleration)
            {
                critical_states[critical_states.size()-2]   = DMPState(trajectory[traj_size-2].getX(), rt_assertor);
            }
            else
            {
 ...
 * NON-REAL-TIME!!!
 */
DMPUnrollInitParams::DMPUnrollInitParams(const Trajectory& trajectory,
                                         RealTimeAssertor* real_time_assertor,
                                         bool is_zeroing_out_velocity_and_acceleration):
    rt_assertor(real_time_assertor)
{
    ...
    if (traj_size >= 2)
    {
        // if the trajectory is containing the "correct" information of the trajectory timing,
