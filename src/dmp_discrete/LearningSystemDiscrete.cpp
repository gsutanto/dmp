 ...
 */
LearningSystemDiscrete::LearningSystemDiscrete(uint dmp_num_dimensions_init, uint model_size_init,
    ...
    LearningSystem(dmp_num_dimensions_init, model_size_init, transformation_system_discrete, data_logging_system,
...
bool LearningSystemDiscrete::isValid()
{
    ...
    if (rt_assert(((TransformSystemDiscrete*) transform_sys)->isValid()) == false)
    {
        return false;
    }
    if (rt_assert((rt_assert(learning_method >= _SCHAAL_LWR_METHOD_)) &&
                  (rt_assert(learning_method <= _JPETERS_GLOBAL_LEAST_SQUARES_METHOD_))) == false)
    {
        return false;
    }
 ...
 */
bool LearningSystemDiscrete::learnApproximator(const TrajectorySet& trajectory_set,
                                               ...
                                               double& mean_tau)
{
    ...
    *learned_weights    = ZeroMatrixNxM(dmp_num_dimensions, model_size);
    ...
    uint                N_traj  = trajectory_set.size();   // total number of trajectories in the set
    if (rt_assert(N_traj > 0) == false)
    {
        return false;
    }
    DoubleVectorPtr     tau;
    if (rt_assert(allocateMemoryIfNonRealTime(is_real_time, tau, N_traj)) == false)
    {
        return false;
    }
    ...
    for (uint i=0; i<N_traj; ++i)
    {
        ...
        // if each trajectory is containing the "correct" information of the trajectory timing,
        // then use this timing information to compute tau:
        (*tau)[i]       = trajectory_set[i][traj_size-1].getTime() - trajectory_set[i][0].getTime();
    ...
    /*
     * The following is WHY this function is NON-REAL-TIME: the usage of Eigen's MatrixXd
     * data structures, which are dynamic data structures allocated in the heap
     * by using smart pointers (to preserve stack space),
     * with NO real-time guarantees. May be called by non-real-time threads.
     */
    // Matrix of target/desired forcing function values along/over the demonstrated trajectories (matrix of size dmp_num_dimensionsXN_traj_states):
    MatrixXxXPtr        f_target;
    if (rt_assert(allocateMemoryIfNonRealTime(is_real_time, f_target,
                                              dmp_num_dimensions, N_traj_states)) == false)
    {
        return false;
    }

    // Matrix of fitted forcing function values along/over the demonstrated trajectories
    // (matrix of size dmp_num_dimensionsXN_traj_states,
    //  used to check regression/learning quality):
    MatrixXxXPtr        f_fit;
    if (rt_assert(allocateMemoryIfNonRealTime(is_real_time, f_fit,
                                              dmp_num_dimensions, N_traj_states)) == false)
    {
        return false;
    }
    ...
    // each column of this matrix being such vector for a particular state in the trajectory)
    // (matrix of size model_sizeXN_traj_states):
    MatrixXxXPtr        Psi;
    if (rt_assert(allocateMemoryIfNonRealTime(is_real_time, Psi,
                                              model_size, N_traj_states)) == false)
    {
        return false;
    }
    ...
    MatrixXxXPtr        Xi;
    if (rt_assert(allocateMemoryIfNonRealTime(is_real_time, Xi,
                                              1, N_traj_states)) == false)
    {
        return false;
    }
    ...
    // along/over the demonstrated trajectories
    // (each column of this matrix being such vector for a particular state in the trajectory)
    // (normalized by the sum of all basis function values in the vector
    //  and modulated/multiplied by the phase (canonical system multiplier) variable value,
    //  for each column in this matrix)
    // (matrix of size model_sizeXN_traj_states):
    MatrixXxXPtr        Phi;
    if (rt_assert(allocateMemoryIfNonRealTime(is_real_time, Phi,
                                              model_size, N_traj_states)) == false)
    {
        return false;
    }

    // Vector of normalized basis function values, modulated by a canonical variable, on a particular state/point in the trajectory (vector of size model_size):
    ...
    FuncApproximatorDiscrete*    func_approx_discr  = ((FuncApproximatorDiscrete*) (transform_sys->getFuncApproxPointer()));
    ...
    for (uint i=0; i<N_traj; i++)
    {
        ...
        if (rt_assert(transform_sys->getTauSystemPointer()->setTauBase((*tau)[i])) == false)
        {
            return false;
        }

        // (re-)start the canonical system:
        if (rt_assert(transform_sys->getCanonicalSystemPointer()->start()) == false)    // Reset canonical system
        {
            return false;
        }
        ...
        if (rt_assert(transform_sys->start(start_state_demo, goal_steady_state_demo)) == false)
        {
            return false;
        }
        ...
        A_learn_average += (goal_steady_state_demo.getX() - start_state_demo.getX());
        ...
        for (uint j = 0; j < traj_size; j++)
        {
            ...
            else
            {
                ...
                dt  = trajectory_set[i][j].getTime() - trajectory_set[i][j-1].getTime();
            }
            if (dt == 0.0)
            {
            ...
            if (rt_assert(transform_sys->getTargetForcingTerm(current_state_demo,
                                                              current_f_target)) == false)
            {
                return false;
            }
            ...
            if (rt_assert(func_approx_discr->getFuncApproxLearningComponents(current_psi,
                                                                             current_sum_psi,
                                                                             current_xi)) == false)
            {
                return false;
            ...
            if (rt_assert(transform_sys->getCanonicalSystemPointer()->updateCanonicalState(dt)) == false)
            {
                return false;
            }
    ...
    if (rt_assert(((TransformSystemDiscrete*) transform_sys)->setLearningAmplitude(A_learn_average)) == false)
    {
        return false;
    }
    ...
    if (learning_method == _SCHAAL_LWR_METHOD_)
    {
        ...
        if (rt_assert(allocateMemoryIfNonRealTime(is_real_time, Xi_squared,
                                                  1, N_traj_states)) == false)
        {
            return false;
        }
        ...
        VectorMPtr          sx2;
        if (rt_assert(allocateMemoryIfNonRealTime(is_real_time, sx2,
                                                  model_size, 1)) == false)
        {
            return false;
        }
        ...
        VectorMPtr          sxtd;
        if (rt_assert(allocateMemoryIfNonRealTime(is_real_time, sxtd,
                                                  model_size, 1)) == false)
        {
            return false;
        }
        ...
        if (rt_assert(allocateMemoryIfNonRealTime(is_real_time, Xi_times_f_target_1D,
                                                  1, N_traj_states)) == false)
        {
            return false;
        }
        ...
        *sx2                = ((Xi_squared->replicate(model_size, 1)).cwiseProduct(*Psi)).rowwise().sum();
        // adapted from Stefan Schaal's MATLAB implementation:
        *sx2                = *sx2 + (1.e-10 * OnesVectorM(model_size));

        for (uint d=0; d<dmp_num_dimensions; d++)
        {
            *Xi_times_f_target_1D   = Xi->cwiseProduct(f_target->block(d, 0, 1, N_traj_states));
            *sxtd                   = ((Xi_times_f_target_1D->replicate(model_size, 1)).cwiseProduct(*Psi)).rowwise().sum();

            learned_weights->block(d, 0, 1, model_size) = (sxtd->cwiseQuotient(*sx2)).transpose();
        }
    }
    else if (learning_method == _JPETERS_GLOBAL_LEAST_SQUARES_METHOD_)
    {
        // R Matrix: the regularization matrix (diagonal matrix of size model_sizeXmodel_size):
        MatrixXxXPtr        R;
        if (rt_assert(allocateMemoryIfNonRealTime(is_real_time, R,
                                                  model_size, model_size)) == false)
        {
            return false;
        }

        // Matrix [(Phi * Phi_transpose) + R] (matrix of size model_sizeXmodel_size):
        MatrixXxXPtr        Phi_Phi_T_plus_R;
        if (rt_assert(allocateMemoryIfNonRealTime(is_real_time, Phi_Phi_T_plus_R,
                                                  model_size, model_size)) == false)
        {
            return false;
        }

        // Pseudo-Inverse of [(Phi * Phi_transpose) + R] (matrix of size model_sizeXmodel_size):
        MatrixXxXPtr        pinv_Phi_Phi_T_plus_R;
        if (rt_assert(allocateMemoryIfNonRealTime(is_real_time, pinv_Phi_Phi_T_plus_R,
                                                  model_size, model_size)) == false)
        {
            return false;
        }
        ...
        *Phi_Phi_T_plus_R   = ((*Phi) * (Phi->transpose())) + (*R);
        ...
        if (rt_assert(pseudoInverse((*Phi_Phi_T_plus_R), (*pinv_Phi_Phi_T_plus_R))) == false)
        {
            return false;
        }
        *learned_weights    = (*f_target) * (Phi->transpose()) * (*pinv_Phi_Phi_T_plus_R);
    }
    ...
    *f_fit              = (*learned_weights) * (*Phi);
    ...
    {
        if (rt_assert(data_logger->writeMatrixToFile(data_directory_path,
                                                     "f_target.txt", *f_target)) == false)
        {
            return false;
        }
        if (rt_assert(data_logger->writeMatrixToFile(data_directory_path,
                                                     "f_fit.txt", *f_fit)) == false)
        {
            return false;
        }
        if (rt_assert(data_logger->writeMatrixToFile(data_directory_path,
                                                     "learned_weights.txt", *learned_weights)) == false)
        {
            return false;
        }
    }
 ...
 *         1=_JPETERS_GLOBAL_LEAST_SQUARES_METHOD_: Jan Peters' Global Least Squares method
 */
uint LearningSystemDiscrete::getLearningMethod()
{
