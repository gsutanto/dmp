    ...
    basis_matrix(new Matrix3xM(3, MAX_MODEL_SIZE))
{
    ...
    allocateMemoryIfNonRealTime(is_real_time, cctraj_old_coord_H_matrix,
                                4, cart_trajectory_size);
    allocateMemoryIfNonRealTime(is_real_time, cctrajd_old_coord_matrix,
                                3, cart_trajectory_size);
    allocateMemoryIfNonRealTime(is_real_time, cctrajdd_old_coord_matrix,
                                3, cart_trajectory_size);
    allocateMemoryIfNonRealTime(is_real_time, cctraj_new_coord_H_matrix,
                                4, cart_trajectory_size);
    allocateMemoryIfNonRealTime(is_real_time, cctrajd_new_coord_matrix,
                                3, cart_trajectory_size);
    allocateMemoryIfNonRealTime(is_real_time, cctrajdd_new_coord_matrix,
                                3, cart_trajectory_size);
}
 ...
 */
CartesianCoordTransformer::CartesianCoordTransformer(RealTimeAssertor* real_time_assertor):
    rt_assertor(real_time_assertor),
    theta_kernel_centers(new VectorM(N_GSUTANTO_LOCAL_COORD_FRAME_BASIS_VECTORS)),
    theta_kernel_Ds(new VectorM(N_GSUTANTO_LOCAL_COORD_FRAME_BASIS_VECTORS)),
    basis_vector_weights_unnormalized(new VectorM(N_GSUTANTO_LOCAL_COORD_FRAME_BASIS_VECTORS)),
    basis_matrix(new Matrix3xM(3, N_GSUTANTO_LOCAL_COORD_FRAME_BASIS_VECTORS))
{
    bool is_real_time       = false;
...
bool CartesianCoordTransformer::reset()
{
    ...
    if (rt_assert((rt_assert(resizeAndReset(cctraj_time_vector,
                                            cart_trajectory_size, 1)))  &&
                  (rt_assert(resizeAndReset(cctraj_old_coord_H_matrix,
                                            4, cart_trajectory_size)))  &&
                  (rt_assert(resizeAndReset(cctrajd_old_coord_matrix,
                                            3, cart_trajectory_size)))  &&
                  (rt_assert(resizeAndReset(cctrajdd_old_coord_matrix,
                                            3, cart_trajectory_size)))  &&
                  (rt_assert(resizeAndReset(cctraj_new_coord_H_matrix,
                                            4, cart_trajectory_size)))  &&
                  (rt_assert(resizeAndReset(cctrajd_new_coord_matrix,
                                            3, cart_trajectory_size)))  &&
                  (rt_assert(resizeAndReset(cctrajdd_new_coord_matrix,
                                            3, cart_trajectory_size)))) == false)
    {
        return false;
    }
...
bool CartesianCoordTransformer::isValid()
{
    ...
    if (rt_assert(isMemoryAllocated(cctraj_time_vector,
                                    cart_trajectory_size, 1)) == false)
    {
        return false;
    }
    if (rt_assert(isMemoryAllocated(cctraj_old_coord_H_matrix,
                                    4, cart_trajectory_size)) == false)
    {
        return false;
    }
    if (rt_assert(isMemoryAllocated(cctrajd_old_coord_matrix,
                                    3, cart_trajectory_size)) == false)
    {
        return false;
    }
    if (rt_assert(isMemoryAllocated(cctrajdd_old_coord_matrix,
                                    3, cart_trajectory_size)) == false)
    {
        return false;
    }
    if (rt_assert(isMemoryAllocated(cctraj_new_coord_H_matrix,
                                    4, cart_trajectory_size)) == false)
    {
        return false;
    }
    if (rt_assert(isMemoryAllocated(cctrajd_new_coord_matrix,
                                    3, cart_trajectory_size)) == false)
    {
        return false;
    }
    if (rt_assert(isMemoryAllocated(cctrajdd_new_coord_matrix,
                                    3, cart_trajectory_size)) == false)
    {
        return false;
    }
    ...
    if (rt_assert((rt_assert(theta_kernel_centers->rows()               == N_GSUTANTO_LOCAL_COORD_FRAME_BASIS_VECTORS)) &&
                  (rt_assert(theta_kernel_Ds->rows()                    == N_GSUTANTO_LOCAL_COORD_FRAME_BASIS_VECTORS)) &&
                  (rt_assert(basis_vector_weights_unnormalized->rows()  == N_GSUTANTO_LOCAL_COORD_FRAME_BASIS_VECTORS)) &&
                  (rt_assert(basis_matrix->rows()                       == 3)) &&
                  (rt_assert(basis_matrix->cols()                       == N_GSUTANTO_LOCAL_COORD_FRAME_BASIS_VECTORS))) == false)
    {
        return false;
    }
    return true;
 ...
 * @return Success or failure
 */
bool CartesianCoordTransformer::computeCTrajCoordTransforms(const Trajectory& cart_trajectory,
                                                            ...
                                                            uint ctraj_local_coordinate_frame_selection)
{
    ...
    if (rt_assert((rt_assert(cart_trajectory.size() >= 2)) &&
                  (rt_assert(cart_trajectory.size() <= MAX_TRAJ_SIZE))) == false)
    {
        return false;
    }
    if (rt_assert(((ctraj_local_coordinate_frame_selection == 3) && (cart_trajectory.size() < 3)) == false) == false)
    {
        return false;
    }
    if (rt_assert((rt_assert(cart_trajectory[0].isValid())) &&
                  (rt_assert(cart_trajectory[cart_trajectory.size()-1].isValid()))) == false)
    {
        return false;
    }
    if (rt_assert((rt_assert(cart_trajectory[0].getDMPNumDimensions() == 3)) &&
                  (rt_assert(cart_trajectory[cart_trajectory.size()-1].getDMPNumDimensions() == 3))) == false)
    {
        return false;
    }
    if (rt_assert((rt_assert(ctraj_local_coordinate_frame_selection >= MIN_CTRAJ_LOCAL_COORD_OPTION_NO)) &&
                  (rt_assert(ctraj_local_coordinate_frame_selection <= MAX_CTRAJ_LOCAL_COORD_OPTION_NO))) == false)
    {
        return false;
    }

    Vector3 start_cart_position = cart_trajectory[0].getX().block(0,0,3,1);
    Vector3 goal_cart_position  = cart_trajectory[cart_trajectory.size()-1].getX().block(0,0,3,1);
    ...
    if (ctraj_local_coordinate_frame_selection == _NO_LOCAL_COORD_FRAME_)           // no local coordinate transformation (or local coord sys = global coord sys)
    {
    ...
    else if (ctraj_local_coordinate_frame_selection == _GSUTANTO_LOCAL_COORD_FRAME_)// G. Sutanto's trajectory local coordinate system formulation
    {
        local_x_axis            = goal_cart_position - start_cart_position;
        local_x_axis            = local_x_axis/local_x_axis.norm();     // normalize local x-axis
        ...
        *theta_kernel_centers   = Eigen::VectorXd::LinSpaced(N_GSUTANTO_LOCAL_COORD_FRAME_BASIS_VECTORS, 0, M_PI);
        for (uint i=0; i<N_GSUTANTO_LOCAL_COORD_FRAME_BASIS_VECTORS; i++)
        {
            if (i < (N_GSUTANTO_LOCAL_COORD_FRAME_BASIS_VECTORS-1))
            {
                (*theta_kernel_Ds)(i,0) = (*theta_kernel_centers)(i+1,0) - (*theta_kernel_centers)(i,0);
            }
            else if (i == (N_GSUTANTO_LOCAL_COORD_FRAME_BASIS_VECTORS-1))
            {
                (*theta_kernel_Ds)(i,0) = (*theta_kernel_centers)(i,0) - (*theta_kernel_centers)(i-1,0);
            }
        }
        *theta_kernel_Ds        = (0.55 * theta_kernel_Ds->array()).array().square().cwiseInverse();

        // compute the kernel/basis vector weights:
        for (uint i=0; i<N_GSUTANTO_LOCAL_COORD_FRAME_BASIS_VECTORS; i++)
        {
            (*basis_vector_weights_unnormalized)(i,0)   = exp(-0.5 * pow((theta - (*theta_kernel_centers)(i,0)), 2) * (*theta_kernel_Ds)(i,0));
        }
        ...
        anchor_local_z_axis = (*basis_matrix) * (*basis_vector_weights_unnormalized)/(basis_vector_weights_unnormalized->colwise().sum())(0,0);
        anchor_local_z_axis = anchor_local_z_axis/(anchor_local_z_axis.norm()); // normalize the anchor_local_z_axis

        local_y_axis        = anchor_local_z_axis.cross(local_x_axis);
        local_y_axis        = local_y_axis/(local_y_axis.norm());               // normalize local y-axis
        local_z_axis        = local_x_axis.cross(local_y_axis);
        local_z_axis        = local_z_axis/(local_z_axis.norm());               // normalize the real local z-axis
    }
    else if (ctraj_local_coordinate_frame_selection == _SCHAAL_LOCAL_COORD_FRAME_)  // Schaal's trajectory local coordinate system formulation
    {
        /*
        ...
        if (fabs(local_x_axis.dot(global_z_axis)) <= PARALLEL_VECTOR_PROJECTION_THRESHOLD)
        {
            local_z_axis        = global_z_axis;
            local_y_axis        = local_z_axis.cross(local_x_axis);
            local_y_axis        = local_y_axis/local_y_axis.norm();     // normalize local y-axis
            local_z_axis        = local_x_axis.cross(local_y_axis);
            local_z_axis        = local_z_axis/local_z_axis.norm();     // normalize local z-axis
        }
    ...
    else if (ctraj_local_coordinate_frame_selection == _KROEMER_LOCAL_COORD_FRAME_) // Kroemer's trajectory local coordinate system formulation
    {
        /*
         ...
         */
        Vector3 approaching_goal_cart_position  = cart_trajectory[cart_trajectory.size()-2].getX().block(0,0,3,1);
        local_x_axis            = goal_cart_position - approaching_goal_cart_position;
        local_x_axis            = local_x_axis/local_x_axis.norm();     // normalize local x-axis
        local_y_axis            = goal_cart_position - start_cart_position;
        local_y_axis            = local_y_axis/local_y_axis.norm();     // normalize "temporary" local y-axis
        // check if local x-axis is "too parallel" with the "temporary" local y-axis; if not:
        if (fabs(local_x_axis.dot(local_y_axis)) <= PARALLEL_VECTOR_PROJECTION_THRESHOLD)
        {
            local_z_axis        = local_x_axis.cross(local_y_axis);
            local_z_axis        = local_z_axis/local_z_axis.norm();     // normalize local z-axis
            local_y_axis        = local_z_axis.cross(local_x_axis);
    ...
    rel_homogen_transform_matrix_local_to_global.block(0, 0, 3, 3)  << rotation_matrix;
    rel_homogen_transform_matrix_local_to_global.block(0, 3, 3, 1)  << translation_vector;
    rel_homogen_transform_matrix_local_to_global.block(3, 0, 1, 4)  << 0.0, 0.0, 0.0, 1.0;

    rel_homogen_transform_matrix_global_to_local.block(0, 0, 3, 3)  << rotation_matrix.transpose();
    rel_homogen_transform_matrix_global_to_local.block(0, 3, 3, 1)  << -(rotation_matrix.transpose()) * translation_vector;
    rel_homogen_transform_matrix_global_to_local.block(3, 0, 1, 4)  << 0.0, 0.0, 0.0, 1.0;
 ...
 * @return Success or failure
 */
bool CartesianCoordTransformer::convertCTrajDMPStatesToMatrices(const Trajectory& cart_trajectory,
                                                                ...
                                                                VectorT& ctraj_time_vector)
{
    ...
    if (rt_assert((rt_assert(ctraj_H_matrix.cols()      == cart_trajectory_size)) &&
                  ...
                  (rt_assert(ctraj_time_vector.rows()   == cart_trajectory_size))) == false)
    {
        return false;
    }
    ...
    for (int i=0; i<cart_trajectory_size; i++)
    {
        ...
        ctraj_H_matrix.block(0,i,3,1)   = cart_trajectory[i].getX().block(0,0,3,1);
        ctrajd_matrix.block(0,i,3,1)    = cart_trajectory[i].getXd().block(0,0,3,1);
        ctrajdd_matrix.block(0,i,3,1)   = cart_trajectory[i].getXdd().block(0,0,3,1);
 ...
 * @return Success or failure
 */
bool CartesianCoordTransformer::computeCTrajAtNewCoordSys(const Matrix4xT& ctraj_H_matrix_old,
                                                          ...
                                                          Matrix3xT& ctrajdd_matrix_new)
{
    ...
    if (rt_assert((rt_assert(ctraj_H_matrix_old.cols()  == cart_trajectory_size)) &&
                  ...
                  (rt_assert(ctrajd_matrix_new.cols()   == cart_trajectory_size))) == false)
    {
        return false;
    }
    if (rt_assert((rt_assert(ctraj_H_matrix_old.isZero() == false)) &&
                  (rt_assert(rel_homogen_transform_matrix_old_to_new.isZero() == false))) == false)
    {
        return false;
    }

    ctraj_H_matrix_new  = rel_homogen_transform_matrix_old_to_new                   * ctraj_H_matrix_old;
    ctrajd_matrix_new   = rel_homogen_transform_matrix_old_to_new.block(0,0,3,3)    * ctrajd_matrix_old;
    ctrajdd_matrix_new  = rel_homogen_transform_matrix_old_to_new.block(0,0,3,3)    * ctrajdd_matrix_old;
 ...
 * @return Success or failure
 */
bool CartesianCoordTransformer::computeCTrajAtNewCoordSys(Vector4* cart_H_old,
                                                          Vector3* cartd_old,
                                                          Vector3* cartdd_old,
                                                          ...
                                                          Vector3* cart_vector_new)
{
    ...
    if (cartd_old)
    {
        ...
        (*cartd_new)    = rel_homogen_transform_matrix_old_to_new.block<3,3>(0,0)   * (*cartd_old);
    }
    ...
    if (cartdd_old)
    {
        ...
        (*cartdd_new)   = rel_homogen_transform_matrix_old_to_new.block<3,3>(0,0)   * (*cartdd_old);
    }
    ...
    if (cart_vector_old)
    {
        ...
        (*cart_vector_new)  = rel_homogen_transform_matrix_old_to_new.block<3,3>(0,0) * (*cart_vector_old);
    }
 ...
 * @return Success or failure
 */
bool CartesianCoordTransformer::convertCTrajMatricesToDMPStates(const Matrix4xT& ctraj_H_matrix,
 ...
 * @return
 */
bool CartesianCoordTransformer::convertCTrajAtOldToNewCoordSys(const Trajectory& cart_trajectory_old_vector,
                                                               const Matrix4x4& rel_homogen_transform_matrix_old_to_new,
                                                               Trajectory& cart_trajectory_new_vector)
{
    ...
    if (rt_assert(cart_trajectory_old_vector.size() == cart_trajectory_size) == false)
    {
        return false;
    }
    if (rt_assert(rel_homogen_transform_matrix_old_to_new.isZero() == false) == false)
    ...
    if (rt_assert(convertCTrajDMPStatesToMatrices(cart_trajectory_old_vector,
                                                  *cctraj_old_coord_H_matrix,
                                                  *cctrajd_old_coord_matrix,
                                                  *cctrajdd_old_coord_matrix,
                                                  *cctraj_time_vector)) == false)
    {
        return false;
    }
    if (rt_assert(computeCTrajAtNewCoordSys(*cctraj_old_coord_H_matrix,
                                            *cctrajd_old_coord_matrix,
                                            *cctrajdd_old_coord_matrix,
                                            rel_homogen_transform_matrix_old_to_new,
                                            *cctraj_new_coord_H_matrix,
                                            *cctrajd_new_coord_matrix,
                                            *cctrajdd_new_coord_matrix)) == false)
    {
        return false;
    }
    if (rt_assert(convertCTrajMatricesToDMPStates(*cctraj_new_coord_H_matrix,
                                                  *cctrajd_new_coord_matrix,
                                                  *cctrajdd_new_coord_matrix,
                                                  *cctraj_time_vector,
 ...
 * @return
 */
bool CartesianCoordTransformer::convertCTrajAtOldToNewCoordSys(const DMPState& dmpstate_old,
                                                               const Matrix4x4& rel_homogen_transform_matrix_old_to_new,
                                                               DMPState& dmpstate_new)
{
    ...
    if (rt_assert((rt_assert(dmpstate_old.getDMPNumDimensions() == 3)) &&
                  (rt_assert(dmpstate_new.getDMPNumDimensions() == 3))) == false)
    {
        return false;
    }
    ...
    if (rt_assert(computeCTrajAtNewCoordSys(&cart_H_old, &cartd_old, &cartdd_old, NULL,
                                            rel_homogen_transform_matrix_old_to_new,
                                            &cart_H_new, &cartd_new, &cartdd_new, NULL)) == false)
    {
        return false;
    }
