...
PMNN::PMNN() : num_dimensions(0), data_io(DataIO()), rt_assertor(nullptr) {
  weights.resize(0);
  biases.resize(0);
}
...
bool PMNN::loadParams(const char* dir_path, uint start_idx) {
  ...
  for (uint dim_idx = start_idx; dim_idx < (start_idx + num_dimensions);
    ...
    snprintf(var_weight_file_path, sizeof(var_weight_file_path), "%s/%u/w%u",
             dir_path, dim_idx, weight_file_counter - 1);
    ...
    for (uint weight_file_count = 0; weight_file_count < weight_file_counter;
         ++weight_file_count) {
      snprintf(var_weight_file_path, sizeof(var_weight_file_path), "%s/%u/w%u",
               dir_path, dim_idx, weight_file_count);
    ...
    snprintf(var_bias_file_path, sizeof(var_bias_file_path), "%s/%u/b%u",
             dir_path, dim_idx, bias_file_counter - 1);
    ...
    for (uint bias_file_count = 0; bias_file_count < bias_file_counter;
         ++bias_file_count) {
      snprintf(var_bias_file_path, sizeof(var_bias_file_path), "%s/%u/b%u",
               dir_path, dim_idx, bias_file_count);
      if (rt_assert(data_io.readMatrixFromFile(
