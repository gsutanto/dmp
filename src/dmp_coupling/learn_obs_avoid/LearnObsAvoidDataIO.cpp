...
LearnObsAvoidDataIO::LearnObsAvoidDataIO()
    : DataIO(),
      feature_trajectory_buffer(new MatrixloaTxF3()),
...
/**
 * Initialize/reset the data buffers and counters (usually done everytime before
...
bool LearnObsAvoidDataIO::reset() {
  if (rt_assert(
          (rt_assert(resizeAndReset(feature_trajectory_buffer, MAX_TRAJ_SIZE,
                                    MAX_LOA_NUM_FEATURE_VECTOR_SIZE))) &&
          (rt_assert(
              resizeAndReset(endeff_cart_position_local_trajectory_buffer,
                             MAX_TRAJ_SIZE, 3))) &&
          (rt_assert(
              resizeAndReset(endeff_cart_velocity_local_trajectory_buffer,
                             MAX_TRAJ_SIZE, 3))) &&
          (rt_assert(
              resizeAndReset(obstacle_cart_position_local_trajectory_buffer,
                             MAX_TRAJ_SIZE, 3))) &&
          (rt_assert(
              resizeAndReset(obstacle_cart_velocity_local_trajectory_buffer,
 ...
 * for sphere obstacles:\n demonstrations, sphere centers, radius in global
 * coordinate system.
 *
 ...
 * system
 * @param obs_sph_centers_cart_position_global [to-be-filled/returned] Set of
 * center point coordinates of \n the spherical obstacles, \n in Cartesian
 * global coordinate system, \n corresponding to a set of demonstrations
 * @param obs_sph_radius [to-be-filled/returned] Set of radius of the spherical
 * obstacles, corresponding to a set of demonstrations
 ...
 * @return Success or failure
 */
bool LearnObsAvoidDataIO::extractObstacleAvoidanceTrainingDataSphObs(
    const char* in_data_dir_path,
    DemonstrationGroupSet& demo_group_set_obs_avoid_global,
    Points& obs_sph_centers_cart_position_global, DoubleVector& obs_sph_radius,
  ...
  if (rt_assert(DataIO::extractSetCartCoordDemonstrationGroups(
          in_data_dir_path, demo_group_set_obs_avoid_global, N_demo_settings,
  ...
  if (rt_assert(rt_assert(obs_sph_centers_cart_position_global.size() ==
                          N_demo_settings) &&
  ...
  for (uint n = 1; n <= N_demo_settings; n++) {
    if (selected_obs_avoid_setting_numbers == nullptr)  // use all settings
    {
      ...
      snprintf(obs_setting_dir_path, sizeof(obs_setting_dir_path), "%s/%u/",
               in_data_dir_path, (*selected_obs_avoid_setting_numbers)[n - 1]);
    }
    // Load sphere obstacle parameters of the demonstration setting:
    if (rt_assert(LearnObsAvoidDataIO::readSphereObstacleParametersFromFile(
            obs_setting_dir_path, obs_sph_centers_cart_position_global[n - 1],
 ...
 * @return Success or failure
 */
bool LearnObsAvoidDataIO::extractObstacleAvoidanceTrainingDataVicon(
    const char* in_data_dir_path,
    DemonstrationGroupSet& demo_group_set_obs_avoid_global,
  ...
  if (rt_assert(vector_point_obstacles_cart_position_global.size() ==
  ...
  for (uint n = 1; n <= N_demo_settings; n++) {
    ...
    if (rt_assert(LearnObsAvoidDataIO::readPointObstaclesFromFile(
            obs_setting_dir_path,
 ...
 * @param endeff_cart_state_local End-effector local Cartesian state that will
 * be copied/saved
 * @param obstacle_cart_state_local Obstacle local Cartesian state that will be
 * copied/saved
 * @return Success or failure
 */
bool LearnObsAvoidDataIO::collectTrajectoryDataSet(
    const VectorloaF3& feature_vector, const DMPState& endeff_cart_state_local,
  ...
  feature_trajectory_buffer->block(trajectory_step_count, 0, 1,
                                   feature_vector_size) =
      feature_vector.transpose();
  endeff_cart_position_local_trajectory_buffer->block(trajectory_step_count, 0,
                                                      1, 3) =
      endeff_cart_state_local.getX().block(0, 0, 3, 1).transpose();
  endeff_cart_velocity_local_trajectory_buffer->block(trajectory_step_count, 0,
                                                      1, 3) =
      endeff_cart_state_local.getXd().block(0, 0, 3, 1).transpose();
  obstacle_cart_position_local_trajectory_buffer->block(trajectory_step_count,
                                                        0, 1, 3) =
      obstacle_cart_state_local.getX().block(0, 0, 3, 1).transpose();
  obstacle_cart_velocity_local_trajectory_buffer->block(trajectory_step_count,
                                                        0, 1, 3) =
      obstacle_cart_state_local.getXd().block(0, 0, 3, 1).transpose();
...
bool LearnObsAvoidDataIO::saveTrajectoryDataSet(const char* data_directory) {
  ...
  if (rt_assert(DataIO::saveTrajectoryData(
          data_directory, "feature_trajectory.txt", trajectory_step_count,
          feature_vector_size, *save_data_buffer,
          *feature_trajectory_buffer)) == false) {
    return false;
  }
  if (rt_assert(DataIO::saveTrajectoryData(
          data_directory, "endeff_cart_position_local_trajectory.txt",
          trajectory_step_count, 3, *save_data_buffer,
          *endeff_cart_position_local_trajectory_buffer)) == false) {
    return false;
  }
  if (rt_assert(DataIO::saveTrajectoryData(
          data_directory, "endeff_cart_velocity_local_trajectory.txt",
          trajectory_step_count, 3, *save_data_buffer,
          *endeff_cart_velocity_local_trajectory_buffer)) == false) {
    return false;
  }
  if (rt_assert(DataIO::saveTrajectoryData(
          data_directory, "obstacle_cart_position_local_trajectory.txt",
          trajectory_step_count, 3, *save_data_buffer,
          *obstacle_cart_position_local_trajectory_buffer)) == false) {
    return false;
  }
  if (rt_assert(DataIO::saveTrajectoryData(
          data_directory, "obstacle_cart_velocity_local_trajectory.txt",
          trajectory_step_count, 3, *save_data_buffer,
 ...
 * @return Success or failure
 */
bool LearnObsAvoidDataIO::readSphereObstacleParametersFromFile(
    const char* dir_path, Vector3& obsctr_cart_position_global,
    double& radius) {
  // Load obstacle coordinate of the demonstration setting:
  if (rt_assert(LearnObsAvoidDataIO::readMatrixFromFile(
          dir_path, "obs_sph_center_coord.txt", obsctr_cart_position_global)) ==
      false) {
    return false;
  }
  // Load the sphere obstacle radius of the demonstration setting:
  if (rt_assert(LearnObsAvoidDataIO::readMatrixFromFile(
          dir_path, "obs_sph_radius.txt", radius)) == false) {
    return false;
  }
 ...
 * @return Success or failure
 */
bool LearnObsAvoidDataIO::readPointObstaclesFromFile(
    const char* dir_path, Points& point_obstacles_cart_position_global) {
  char file_path[1000];
  snprintf(file_path, sizeof(file_path), "%s/%s", dir_path,
           "obs_markers_global_coord.txt");

  // Load point obstacles' coordinates of the demonstration setting:
  if (rt_assert(LearnObsAvoidDataIO::extract3DPoints(
          file_path, point_obstacles_cart_position_global, false)) == false) {
    return false;
  }
