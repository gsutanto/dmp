 ...
 */
TransformCouplingLearnTactileFeedback::TransformCouplingLearnTactileFeedback(
    uint dmp_num_dimensions_init, uint start_index_init,
    uint pmnn_input_num_dimensions_init, uint pmnn_phase_kernel_model_size_init,
    uint pmnn_output_num_dimensions_init, VectorNN_N* pmnn_input_vector_ptr,
    VectorNN_N* pmnn_phase_kernel_modulation_ptr,
    VectorNN_N* pmnn_output_vector_ptr, RealTimeAssertor* real_time_assertor)
...
/**
 * Checks whether this transformation coupling term for learning tactile
 * feedback by demonstration is valid or not.
 *
...
bool TransformCouplingLearnTactileFeedback::isValid() {
  ...
  if (rt_assert(
          (rt_assert(pmnn_input_vector->rows() == pmnn_input_num_dimensions)) &&
          (rt_assert(pmnn_phase_kernel_modulation->rows() ==
                     pmnn_phase_kernel_model_size)) &&
          (rt_assert(pmnn_output_vector->rows() ==
                     pmnn_output_num_dimensions))) == false) {
    return false;
  }
  return true;
}
 ...
 */
bool TransformCouplingLearnTactileFeedback::getValue(VectorN& ct_acc,
                                                     VectorN& ct_vel) {
  ...
  if (rt_assert(pmnn->computePrediction(
          *pmnn_input_vector, *pmnn_phase_kernel_modulation,
          *pmnn_output_vector, start_index,
          start_index + dmp_num_dimensions - 1)) == false) {
    return false;
  }

  ct_acc.block(0, 0, dmp_num_dimensions, 1) =
      pmnn_output_vector->block(start_index, 0, dmp_num_dimensions, 1);
