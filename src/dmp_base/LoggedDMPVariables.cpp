...
LoggedDMPVariables::LoggedDMPVariables():
    dmp_num_dimensions(0), model_size(0), rt_assertor(NULL),
    tau(0.0), transform_sys_state_local(DMPState()), transform_sys_state_global(DMPState()),
    goal_state_local(DMPState()), goal_state_global(DMPState()),
...
LoggedDMPVariables::LoggedDMPVariables(uint dmp_num_dimensions_init,
    ...
    dmp_num_dimensions(dmp_num_dimensions_init), model_size(num_basis_functions),
    rt_assertor(real_time_assertor),
    tau(0.0), transform_sys_state_local(DMPState(dmp_num_dimensions, rt_assertor)),
...
bool LoggedDMPVariables::isValid() const
{
    ...
    if (rt_assert((rt_assert(dmp_num_dimensions == transform_sys_state_local.getDMPNumDimensions()))    &&
                  (rt_assert(dmp_num_dimensions == transform_sys_state_global.getDMPNumDimensions()))   &&
                  (rt_assert(dmp_num_dimensions == goal_state_local.getDMPNumDimensions()))             &&
                  (rt_assert(dmp_num_dimensions == goal_state_global.getDMPNumDimensions()))            &&
                  (rt_assert(dmp_num_dimensions == steady_state_goal_position_local.rows()))            &&
                  (rt_assert(dmp_num_dimensions == steady_state_goal_position_global.rows()))           &&
                  (rt_assert(model_size         == basis_functions.rows()))                             &&
                  (rt_assert(dmp_num_dimensions == forcing_term.rows()))                                &&
                  (rt_assert(dmp_num_dimensions == transform_sys_ct_acc.rows()))) == false)
    {
        return false;
    }
    return true;
...
uint LoggedDMPVariables::getModelSize() const
{
    return (model_size);
}
