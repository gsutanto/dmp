...
void print_usage() {
  printf(
      "Usage: dmp_cart_coord_dmp_single_traj_training_demo [-f "
      ...
      "[-h time_goal_change(>=0)] [-t tau_reproduce(>=0)] [-e "
      "rt_err_file_path]\n");
}
...
int main(int argc, char** argv) {
  ...
  int opt = 0;
  int long_index = 0;
  while ((opt = getopt_long(argc, argv, "f:c:m:r:h:t:e:", long_options,
                            &long_index)) != -1) {
    switch (opt) {
      case 'f':
  ...
  if (rt_assert_main(cart_dmp.learn(
          get_data_path("/cart_dmp/cart_coord_dmp/single_traj_training/"
  ...
  for (uint i = 0; i < (round(time_reproduce_max * task_servo_rate) + 1); ++i) {
    ...
    if (fabs(time - time_goal_change) <
        (5 * epsilon))  // time to execute goal change
    {
