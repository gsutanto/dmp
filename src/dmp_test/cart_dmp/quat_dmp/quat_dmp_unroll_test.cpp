...
void print_usage() {
  printf(
      "Usage: dmp_quat_dmp_unroll_demo [-r time_reproduce_max(>=0)] [-t "
      "tau_reproduce(>=0)] [-e rt_err_file_path]\n");
}
...
int main(int argc, char** argv) {
  ...
  std::string dmp_plot_dir_path_str = 
    get_plot_path("/cart_dmp/quat_dmp/");
  ...
  int opt = 0;
  int long_index = 0;
  while ((opt = getopt_long(argc, argv, "r:t:e:", long_options, &long_index)) !=
         -1) {
    switch (opt) {
      case 'r':
  ...
  QuaternionDMP quat_dmp(model_size, &canonical_sys_discr, learning_method,
  ...
  /***** Start Unrolling Loaded Quaternion DMP with Zero Initial Angular
  ...
  for (uint i = 0; i < (round(time_reproduce_max * task_servo_rate) + 1); ++i) {
    ...
    printf("%.05f %.05f %.05f %.05f %.05f\n", time, current_state.getQ()[0],
           current_state.getQ()[1], current_state.getQ()[2],
           current_state.getQ()[3]);
  }

  /***** End Unrolling Loaded Quaternion DMP with Zero Initial Angular Velocity
  ...
  /***** Start Unrolling Loaded Quaternion DMP with NON-Zero Initial Angular
   ***** Velocity and Angular Acceleration *****/
  ...
  /***** End Unrolling Loaded Quaternion DMP with NON-Zero Initial Angular
   ***** Velocity and Angular Acceleration *****/
