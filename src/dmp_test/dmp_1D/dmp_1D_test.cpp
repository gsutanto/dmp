...
int main(int argc, char** argv)
{
    ...
    int     opt                     = 0;
    int     long_index              = 0;
    while ((opt = getopt_long(argc, argv,"f:c:m:r:h:g:t:e:", long_options, &long_index )) != -1)
    {
        switch (opt)
        {
    ...
    // Initialize DMP
    DMPDiscrete1D                   dmp_discrete_1D(model_size, &can_sys_discr,
                                                    formula_type, learning_method,
                                                    &rt_assertor, dmp_plot_dir_path);

    // Learn the trajectory
    if (rt_assert_main(dmp_discrete_1D.learn(get_data_path("/dmp_1D/sample_traj_1.txt").c_str(),
    ...
    /*if (rt_assert_main(dmp_discrete_1D.saveBasisFunctions("../plot/dmp_1D/sample_dmp_basis_functions.txt")) == false)
    {
        return (-1);
    }*/
    // test weight-saving:
    if (rt_assert_main(dmp_discrete_1D.saveParams(dmp_plot_dir_path,
                                                  "sample_dmp_1D_learned_weights.txt",
                                                  "sample_dmp_1D_A_learn.txt",
                                                  "sample_dmp_1D_mean_start_position.txt",
    ...
    // test weight-loading:
    if (rt_assert_main(dmp_discrete_1D.loadParams(dmp_plot_dir_path,
                                                  "sample_dmp_1D_learned_weights.txt",
                                                  "sample_dmp_1D_A_learn.txt",
                                                  "sample_dmp_1D_mean_start_position.txt",
                                                  "sample_dmp_1D_mean_goal_position.txt",
    ...
    // Start DMP
    if (rt_assert_main(dmp_discrete_1D.start(dmp_unroll_init_parameters)) == false)
    {
        return (-1);
    }
    ...
    if (rt_assert_main(dmp_discrete_1D.startCollectingTrajectoryDataSet()) == false)
    {
        return (-1);
    }
    ...
    for(uint i = 0; i < (round(time_reproduce_max*task_servo_rate) + 1); i++)
	{
        ...
        if (i == (round(time_goal_change*task_servo_rate) + 1)) // start changing goal at (i/300) seconds
		{
            if (rt_assert_main(dmp_discrete_1D.setNewSteadyStateGoalPosition(new_goal)) == false)
            {
                return (-1);
            }
        }
        ...
        if (rt_assert_main(dmp_discrete_1D.getNextState(dt, true, state)) == false)
        {
            return (-1);
        }
