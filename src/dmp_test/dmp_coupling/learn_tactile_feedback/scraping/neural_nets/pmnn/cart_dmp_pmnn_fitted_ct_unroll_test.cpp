...
void print_usage() {
  printf(
      "Usage: dmp_cart_dmp_pmnn_fitted_ct_unroll_demo [-o "
      "output_plot_dir_path] [-e rt_err_file_path]\n");
}
...
int main(int argc, char** argv) {
  ...
  int opt = 0;
  int long_index = 0;
  while ((opt = getopt_long(argc, argv, "o:e:", long_options, &long_index)) !=
         -1) {
    switch (opt) {
      case 'o':
        snprintf(output_plot_dir_path, sizeof(output_plot_dir_path),
  ...
  if ((formula_type < _SCHAAL_DMP_) || (formula_type > _HOFFMANN_DMP_) ||
      ...
      (tau_reproduce < 0.0) || (strcmp(output_plot_dir_path, "") == 0) ||
  ...
  for (uint s = 0; s < N_sense; ++s) {
    if (s == 0) {
      ...
      sense_start_idx_in_aggreg_feat_vec[s] =
          sense_start_idx_in_aggreg_feat_vec[s - 1] +
          sense_dimensionality[s - 1];
    }
  }
  uint N_total_sense_dimensionality =
      sense_start_idx_in_aggreg_feat_vec[N_sense - 1] +
      sense_dimensionality[N_sense - 1];
  ...
  // start index for each action modality in the aggregated coupling term vector
  // Ct_vector:
  ...
  for (uint a = 0; a < N_act; ++a) {
    if (a == 0) {
      ...
      act_start_idx_in_aggreg_ct_vec[a] =
          act_start_idx_in_aggreg_ct_vec[a - 1] + act_dimensionality[a - 1];
    }
  }
  uint N_total_act_dimensionality =
      act_start_idx_in_aggreg_ct_vec[N_act - 1] + act_dimensionality[N_act - 1];
  ...
  std::vector<std::vector<Trajectory> > dmpd_sense_unroll_critical_states(
      N_sense);
  ...
  std::string prims_param_root_dir_path = get_data_path(
      "/dmp_coupling/learn_tactile_feedback/scraping/learned_prims_params/");
  char var_params_dir_path[1000] = "";
  std::string trial_data_root_dir_path = get_data_path(
      "/dmp_coupling/learn_tactile_feedback/scraping/unroll_test_dataset/"
      "all_prims/");
  ...
  for (uint m = 0; m < N_sense; ++m) {
    ...
    if (rt_assert_main(dmp_discrete_sense[m]->setScalingUsage(
    ...
    for (uint np = 0; np < N_prims; ++np) {
      snprintf(var_params_dir_path, sizeof(var_params_dir_path),
               "%s/sense_%s/prim%u/", prims_param_root_dir_path.c_str(),
               sense_name[m].c_str(), np + 1);
      // Load the Sensory Primitives' parameters:
      if (rt_assert_main(dmp_discrete_sense[m]->loadParams(
              var_params_dir_path, "w", "A_learn", "start", "goal", "tau")) ==
          false) {
        printf(
      ...
      if (rt_assert_main(dmp_discrete_sense[m]->getParams(
              dmpd_sense_weights[m][np], dmpd_sense_A_learn[m][np])) == false) {
        printf(
      ...
      snprintf(var_data_dir_path, sizeof(var_data_dir_path),
               "%s/setting_%u_trial_%u/prim%u/",
               trial_data_root_dir_path.c_str(), setting_no, trial_no, np + 1);
      // Load the "actual" sensor traces:
      std::string file_name = "sense_" + sense_name[m];
      if (rt_assert_main(data_io.readMatrixFromFile(
              var_data_dir_path, file_name.c_str(), X_sensor_trace[m][np])) ==
          false) {
        printf(
      ...
      if (m < 2)  // BioTac electrodes
      {
        if (np == 0) {
          ...
          for (uint t = 0; t < N_points_ave_init_offset; ++t) {
            X_sensor_offset[m][np] =
                X_sensor_offset[m][np] +
                (1.0 * X_sensor_trace[m][np]
                           ->block(t, 0, 1, sense_dimensionality[m])
                           .transpose());
          }
          X_sensor_offset[m][np] =
              (1.0 / N_points_ave_init_offset) * X_sensor_offset[m][np];
        } else {
      ...
      if (m == 0) {
        ...
        if (rt_assert_main(unroll_traj_length[np] ==
                           X_sensor_trace[m][np]->rows()) == false) {
          printf(
  ...
  for (uint a = 0; a < N_act; ++a) {
    ...
    if (rt_assert_main(dmp_discrete_act[a]->setScalingUsage(
    ...
    for (uint np = 0; np < N_prims; ++np) {
      snprintf(var_params_dir_path, sizeof(var_params_dir_path),
               "%s/%s/prim%u/", prims_param_root_dir_path.c_str(),
               act_name[a].c_str(), np + 1);
      // Load the Action Primitives' parameters:
      if (a == 0) {
        if (rt_assert_main(dmp_discrete_act[a]->loadParams(
                var_params_dir_path, "w", "A_learn", "start_global",
                "goal_global", "tau")) == false) {
          printf(
      ...
      // Store the parameters on buffers:
      if (rt_assert_main(dmp_discrete_act[a]->getParams(
              dmpd_act_weights[a][np], dmpd_act_A_learn[a][np])) == false) {
        printf(
  ...
  for (uint np = 0; np < N_prims; ++np) {
    ...
    //        X[np].resize(unroll_traj_length[np],
    //        N_total_sense_dimensionality);
  ...
  for (uint np = 0; np < N_prims; ++np) {
    ...
    for (uint m = 0; m < N_sense; ++m) {
      if (rt_assert_main(dmp_discrete_sense[m]->setParams(
              dmpd_sense_weights[m][np], dmpd_sense_A_learn[m][np])) == false) {
        printf(
      ...
      dmpd_sense_unroll_critical_states[m][np][0] =
          DMPState(dmpd_sense_start[m][np], &rt_assertor);
      dmpd_sense_unroll_critical_states[m][np][1] =
          DMPState(dmpd_sense_goal[m][np], &rt_assertor);

      if (rt_assert_main(dmp_discrete_sense[m]->start(
              dmpd_sense_unroll_critical_states[m][np], unroll_tau[np])) ==
          false) {
        printf(
    ...
    for (uint a = 0; a < N_act; ++a) {
      if (rt_assert_main(dmp_discrete_act[a]->setParams(
              dmpd_act_weights[a][np], dmpd_act_A_learn[a][np])) == false) {
        printf(
      ...
      if (np == 0) {
        if (a == 0) {
          dmpd_cart_coord_unroll_critical_states[np][0] =
              DMPState(dmpd_act_start[a][np], &rt_assertor);
          dmpd_cart_coord_unroll_critical_states[np][1] =
              DMPState(dmpd_act_goal[a][np], &rt_assertor);
        } else  // if (a == 1)
        {
          dmpd_quat_unroll_critical_states[np][0] =
              QuaternionDMPState(dmpd_act_start[a][np], &rt_assertor);
          dmpd_quat_unroll_critical_states[np][1] =
              QuaternionDMPState(dmpd_act_goal[a][np], &rt_assertor);
        }
      } else {
        if (a == 0) {
      ...
      if (a == 0) {
        if (rt_assert_main(dmp_discrete_act[a]->start(
                dmpd_cart_coord_unroll_critical_states[np], unroll_tau[np])) ==
            false) {
          printf(
      ...
      {
        if (rt_assert_main(dmp_discrete_quat->startQuaternionDMP(
                dmpd_quat_unroll_critical_states[np], unroll_tau[np])) ==
            false) {
          printf(
              "Failed starting sensory primitive for action modality %s, "
              "primitive #%u\n",
    ...
    for (uint t = 0; t < unroll_traj_length[np]; ++t) {
      for (uint m = 0; m < N_sense; ++m) {
        ...
        X_vector.block(sense_start_idx_in_aggreg_feat_vec[m], 0,
                       sense_dimensionality[m], 1) =
            (X_sensor_trace[m][np]
                 ->block(t, 0, 1, sense_dimensionality[m])
                 .transpose() -
      ...
      double sum_psi = (func_approx_basis_functions.colwise().sum())(0, 0) +
                       (model_size * 1.e-10);
      normalized_phase_PSI_mult_phase_V =
          func_approx_basis_functions * (canonical_multiplier / sum_psi);
      ...
      for (uint a = 0; a < N_act; ++a) {
        ...
        if (a == 0) {
          if (rt_assert_main(dmp_discrete_act[a]->getNextState(
                  dt, false, cart_coord_state)) == false) {
            printf(
        ...
        {
          if (rt_assert_main(dmp_discrete_quat->getNextQuaternionState(
                  dt, false, quat_state)) == false) {
            printf(
                "Failed action primitive unrolling at time t=%u for action "
                "modality %s, primitive #%u\n",
                t, act_name[a].c_str(), np + 1);
      ...
      pose_traj_unroll[np].block(t, 0, 1, 3) =
          cart_coord_state.getX().block(0, 0, 3, 1).transpose();
      pose_traj_unroll[np].block(t, 3, 1, 4) =
          quat_state.getQ().block(0, 0, 4, 1).transpose();

      // actually the following computePrediction() is unnecessary,
      // since inside the coupling term getValue(), this is called
      // automatically; the following is called only for logging purpose:
      if (rt_assert_main(pmnn[np]->computePrediction(
      ...
      //            X[np].block(t, 0, 1, N_total_sense_dimensionality)  =
      //            X_vector.transpose(); Ct[np].block(t, 0, 1,
      //            N_total_act_dimensionality)   = Ct_vector.transpose();
    }

    //        snprintf(var_file_name, sizeof(var_file_name), "X_prim%u", np+1);
    //        if (rt_assert_main(data_io.writeMatrixToFile(output_plot_dir_path,
    ...
    snprintf(var_file_name, sizeof(var_file_name),
             "test_cpp_coupled_cart_dmp_unroll_test_prim%u", np + 1);
    if (rt_assert_main(data_io.writeMatrixToFile(
            output_plot_dir_path, var_file_name, pose_traj_unroll[np])) ==
        false) {
