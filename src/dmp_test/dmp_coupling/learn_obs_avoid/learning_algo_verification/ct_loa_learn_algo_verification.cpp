...
void print_usage() {
  printf(
      "Usage: dmp_ct_loa_learn_algo_verification_demo [-f "
...
int main(int argc, char** argv) {
  ...
  TCLearnObsAvoidFeatureParameter tc_loa_feat_param(
      ...
      _SPHERE_OBSTACLE_CENTER_RADIUS_FILE_FORMAT_, AF_H14_feature_method, 3, 3,
      (6.0 / M_PI), (14.0 / M_PI), 10.0, 30.0, 1, 7.0, 8.0);
  ...
  snprintf(var_input_file_path, sizeof(var_input_file_path),
           "%s/synthetic_data/1/", loa_data_dir_path);
  ...
  for (uint i = 0; i < (round(tau * task_servo_rate) + 1); ++i) {
    ...
    // IMPORTANT PART here:
    // Here ee_cart_state_global is updated, using the current (or real-time)
    // DMP unrolled trajectory. These variable will in turn be used for
    // computing the features of obstacle avoidance coupling term.
  ...
  // Move synthetically-generated obstacle avoidance demonstration trajectory to
  // the data directory:
  snprintf(
      var_input_file_path, sizeof(var_input_file_path),
      "%s/w_synthetic_obs_avoidance/transform_sys_state_global_trajectory.txt",
      dmp_plot_dir_path);
  snprintf(var_output_file_path, sizeof(var_output_file_path),
  ...
  snprintf(var_input_file_path, sizeof(var_input_file_path),
           "%s/synthetic_data/", loa_data_dir_path);
