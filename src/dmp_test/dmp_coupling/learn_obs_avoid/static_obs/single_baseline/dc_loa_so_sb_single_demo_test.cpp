...
void print_usage()
{
    printf("Usage: amd_clmc_dmp_dc_loa_so_sb_single_demo_demo [-f formula_type(0=_SCHAAL_ OR 1=_HOFFMANN_)] [-c canonical_order(1 OR 2)] [-m learning_method(0=_SCHAAL_LWR_METHOD_ OR 1=_JPETERS_GLOBAL_LEAST_SQUARES_METHOD_)] [-o loa_plot_dir_path] [-e rt_err_file_path]\n");
}
...
int main(int argc, char** argv)
{
    ...
    get_data_path(loa_data_dir_path,"/dmp_coupling/learn_obs_avoid/static_obs/data_sph_new/SubjectNo1/");
    char        loa_plot_dir_path[1000];
    get_plot_path(loa_plot_dir_path,"/dmp_coupling/learn_obs_avoid/feature_trajectory/static_obs/single_baseline/single_demo/");
    ...
    if (rt_assert_main(cart_dmp.saveTrajectoryDataSet(get_plot_path("/dmp_coupling/learn_obs_avoid/tau_invariance_evaluation/wo_obs/").c_str())) == false)
    {
        return (-1);
    }
    ...
    cart_dmp.stopCollectingTrajectoryDataSet();
    if (rt_assert_main(cart_dmp.saveTrajectoryDataSet(get_plot_path("/dmp_coupling/learn_obs_avoid/tau_invariance_evaluation/w_obs_real/").c_str())) == false)
    {
        return (-1);
    }
    transform_coupling_learn_obs_avoid.stopCollectingTrajectoryDataSet();
    if (rt_assert_main(transform_coupling_learn_obs_avoid.saveTrajectoryDataSet()) == false)
    {
        return (-1);
    }
    /****************** with obstacle (END) ******************/
