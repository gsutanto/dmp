...
void print_usage()
{
    printf("Usage: amd_clmc_dmp_ct_loa_so_sb_multi_demo_vicon_PMNN_unrolling_demo [-f formula_type(0=_SCHAAL_ OR 1=_HOFFMANN_)] [-c canonical_order(1 OR 2)] [-m learning_method(0=_SCHAAL_LWR_METHOD_ OR 1=_JPETERS_GLOBAL_LEAST_SQUARES_METHOD_)] [-r regularization_const] [-o loa_plot_dir_path] [-w ct_loa_weights_subpath] [-e rt_err_file_path]\n");
}
...
int main(int argc, char** argv)
{
    ...
    get_plot_path(dmp_plot_dir_path,"/dmp_coupling/learn_obs_avoid/feature_trajectory/static_obs/single_baseline/multi_demo_vicon/unroll_tests/");
    char        ct_loa_weights_subpath[1000];
    strcpy(ct_loa_weights_subpath, "learned_weights/1/learn_obs_avoid_weights_matrix_ARD.txt");
    ...
    int         opt                         = 0;
    int         long_index                  = 0;
    while ((opt = getopt_long( argc, argv,"f:c:m:r:o:w:e:", long_options, &long_index )) != -1)
    {
        switch (opt)
        {
            ...
            case 'o'    : strcpy(loa_plot_dir_path, optarg);
                break;
            case 'w'    : strcpy(ct_loa_weights_subpath, optarg);
                break;
    ...
    if ((formula_type < _SCHAAL_DMP_) || (formula_type > _HOFFMANN_DMP_) ||
        ...
        (strcmp(loa_plot_dir_path, "") == 0) || (strcmp(ct_loa_weights_subpath, "") == 0) ||
    ...
    boost::shared_ptr<PMNN>         pmnn;
    char                            pmnn_model_path[1000]   = "";
    // Initialize Phase-Modulated Neural Network (PMNN):
    pmnn    = boost::make_shared<PMNN>(PMNN_output_size,
                                       topology,
                                       activation_functions,
                                       &rt_assertor);

    sprintf(pmnn_model_path, "%sprim1/",
            get_data_path("/dmp_coupling/learn_obs_avoid/static_obs/neural_nets/pmnn/cpp_models/").c_str());
    ...
    if (rt_assert_main(cart_dmp.setScalingUsage(is_using_scaling_init)) == false)
    {
        return (-1);
    }
    ...
    if (rt_assert_main(allocateMemoryIfNonRealTime(is_real_time, input_demo_group_set_obs_avoid_global)) == false)
    {
        return false;
    }
    ...
    if (rt_assert_main(allocateMemoryIfNonRealTime(is_real_time, vector_point_obstacles_cart_position_global)) == false)
    {
        return false;
    }

    ObstacleStatesVectorPtr         point_obss_cart_state_traj_global;
    if (rt_assert_main(allocateMemoryIfNonRealTime(is_real_time, point_obss_cart_state_traj_global)) == false)
    {
        return false;
    }
    ...
    if (rt_assert_main(allocateMemoryIfNonRealTime(is_real_time, demo_group_set_dmp_unroll_init_params)) == false)
    {
        return false;
    }
    ...
    DMPUnrollInitParams             dmp_unroll_init_parameters_wo_obs(tau_reproduce,
                                                                      DMPState(cart_dmp.getMeanStartPosition(), &rt_assertor),
                                                                      DMPState(cart_dmp.getMeanGoalPosition(), &rt_assertor),
                                                                      &rt_assertor);
    if (rt_assert_main(cart_dmp.start(dmp_unroll_init_parameters_wo_obs)) == false)
    {
        return (-1);
    }
...
//    }
    ...
    if (rt_assert_main((demo_group_set_dmp_unroll_init_params->size() == N_demo_settings) &&
                       (vector_point_obstacles_cart_position_global->size() == N_demo_settings))== false)
    {
        return (-1);
    }
...
//            std::cout << "Unrolling learned obstacle avoidance on Demo Setting #" << (n+1) << "/" << N_demo_settings << ", demo #" << (j+1) << "/" << (*demo_group_set_dmp_unroll_init_params)[n].size() << " ..." << std::endl;
            ...
            // define unrolling parameters:
            DMPUnrollInitParams             dmp_unroll_init_parameters_w_obs(tau_reproduce,
                                                                             DMPState(cart_dmp.getMeanStartPosition(), &rt_assertor),
                                                                             DMPState(cart_dmp.getMeanGoalPosition(), &rt_assertor),
                                                                             &rt_assertor);
            if (rt_assert_main(cart_dmp.start(dmp_unroll_init_parameters_w_obs)) == false)
            {
                return (-1);
            }
            ...
            if (rt_assert_main(transform_coupling_learn_obs_avoid.computePointObstaclesCartStateGlobalTrajectoryFromStaticViconData((*vector_point_obstacles_cart_position_global)[n],
...
//            }
            for (uint i = 1; i <= (round(tau_reproduce*task_servo_rate) + 1); ++i)
            {
                ...
                // i.e. conditioned with features computed with current information.
                ee_cart_state_global                = current_state;
                point_obstacles_cart_state_global   = (*point_obss_cart_state_traj_global)[0];
                goal_cart_position_global           = cart_dmp.getMeanGoalPosition();
