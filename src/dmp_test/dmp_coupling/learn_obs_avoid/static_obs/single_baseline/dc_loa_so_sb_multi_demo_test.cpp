...
void print_usage() {
  printf(
      "Usage: dmp_dc_loa_so_sb_multi_demo_demo [-f formula_type(0=_SCHAAL_ OR "
...
int main(int argc, char** argv) {
  ...
  snprintf(var_output_file_path, sizeof(var_output_file_path),
           "%s/%u/", loa_plot_dir_path, demo_settings_counter);
  ...
  for (uint n = 1; n <= demo_settings_counter; n++) {
    std::cout << "Evaluating LOA Performance on Demo Setting #" << n << "/"
              << demo_settings_counter << " ..." << std::endl;

    snprintf(var_input_file_path, sizeof(var_input_file_path),
             "%s/%u/", loa_data_dir_path, n);
    ...
    // cart_dmp
    std::cout << "Real Case (using TEST trajectory to compute features) ..."
              << std::endl;
    ...
    for (uint i = 0; i < (round(tau_reproduce * task_servo_rate) + 1); ++i) {
      ...
      // Different from the "ideal" case before, here these variables are
      // updated using the current (or real-time) DMP unrolled trajectory. These
      // variable will in turn be used for computing the features of obstacle
      // avoidance coupling term. Thus the resulting obstacle avoidance coupling
      // term is the "realistic" one, i.e. i.e. conditioned with features
      // computed with known information.
