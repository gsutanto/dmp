...
void print_usage() {
  printf(
      "Usage: dmp_ct_loa_so_sb_multi_demo_vicon_NN_unrolling_demo [-f "
...
int main(int argc, char** argv) {
  ...
  get_data_path(
      loa_data_dir_path,
      "/dmp_coupling/learn_obs_avoid/static_obs/data_multi_demo_vicon_static/");
  ...
  snprintf(NN_param_dir_path, sizeof(NN_param_dir_path),
           "%s/trained_NN_params/", loa_data_dir_path);
  ...
  if (rt_assert_main(
          transform_coupling_learn_obs_avoid
              .extractObstacleAvoidanceTrainingDataVicon(
                  loa_data_dir_path, *input_demo_group_set_obs_avoid_global,
  ...
  snprintf(baseline_param_dir_path, sizeof(baseline_param_dir_path),
           "%s/baseline/", loa_plot_dir_path);
  if (rt_assert_main(cart_dmp.loadParams(
          baseline_param_dir_path, "f_baseline_weights_matrix.txt",
  ...
  for (uint n = 0; n < N_demo_settings; n++) {
    for (uint j = 0; j < (*demo_group_set_dmp_unroll_init_params)[n].size();
         j++) {
      std::cout << "Unrolling learned obstacle avoidance on Demo Setting #"
                << (n + 1) << "/" << N_demo_settings << ", demo #" << (j + 1)
