 ...
 *
 * System that represents the current Quaternion DMP goal state (including orientation, angular velocity, angular acceleration,
 * and time (if used)) and its evolution toward the final (steady-state) Quaternion goal position.
 *
...
protected:
    ...
    QuaternionDMPState  current_quaternion_goal_state; // current Quaternion DMP goal state (orientation, angular velocity, angular acceleration, and time (if used))
...
public:
     ...
     */
    QuaternionGoalSystem(TauSystem* tau_system, RealTimeAssertor* real_time_assertor, double alpha_init=25.0/2.0);
     ...
     */
    bool startQuaternionGoalSystem(const QuaternionDMPState& current_goal_state_init, const VectorN& G_init);
