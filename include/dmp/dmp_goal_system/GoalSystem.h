 ...
 * GoalSystem.h
 *
 * System that represents the current DMP goal state (including position, velocity, acceleration,
 * and time (if used)) and its evolution toward the final (steady-state) goal position.
 *
...
protected:
    ...
    uint                dmp_num_dimensions; // number of dimensions in the DMP, for example: Cartesian DMP has 3 dimensions, Quaternion DMP has 3 dimensions, etc.
    double              alpha;

    // The following field(s) are written as RAW pointers for extensibility in Object-Oriented Programming (OOP)
    DMPState*           current_goal_state; // current DMP goal state (position, velocity, acceleration, and time (if used))
    ...
    TauSystem*          tau_sys;            // tau system that computes the tau parameter for this goal system
...
public:
     ...
     * @param real_time_assertor Real-Time Assertor for troubleshooting and debugging
     */
    GoalSystem(uint dmp_num_dimensions_init, DMPState* current_goal_state_ptr, uint goal_num_dimensions_init,
     ...
     */
    virtual bool start(const DMPState& current_goal_state_init, const VectorN& G_init);
