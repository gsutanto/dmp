...
class CanonicalSystemDiscrete : public CanonicalSystem {
 ...
 public:
   ...
   * debugging
   * @param cs_order The order of the dynamical system describing the canonical
   * system
   * @param canonical_couplers All temporal couplings associated with the
   * temporal DMP development (please note that this is a pointer to vector of
   * pointer to CanonicalCoupling data structure, for flexibility and
   * re-usability)
   */
  CanonicalSystemDiscrete(
   ...
   *        Best practice:\n
   *        If order==1, set it to 1/3 of alpha-parameter of the transformation
   * system\n If order==2, set it equal to alpha-parameter of the transformation
   * system (= 25.0)
   * @param real_time_assertor Real-Time Assertor for troubleshooting and
   * debugging
   * @param canonical_couplers All temporal couplings associated with the
   ...
   *        Best practice: set it to 1/4 of alpha-parameter of the canonical
   * system, to get a critically-damped response
   * @param real_time_assertor Real-Time Assertor for troubleshooting and
   * debugging
   * @param canonical_couplers All temporal couplings associated with the
   * temporal DMP development (please note that this is a pointer to vector of
   ...
   *
   * @return Discrete canonical system is valid (true) or discrete canonical
   * system is invalid (false)
   */
  bool isValid();
  ...
  /**
   * Returns the state (position, velocity, and acceleration) of the canonical
   * system.
   */
  Vector3 getCanonicalState();

  /**
   * Runs one time-step ahead of the canonical system and updates the current
   * canonical state based on the time difference dt from the previous
   * execution.
   *
