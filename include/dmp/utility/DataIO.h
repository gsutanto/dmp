...
public:
    ...
    bool write1DTrajectoryToFile(const Trajectory& trajectory,
                                 const char* file_path, bool is_comma_separated=true);
     ...
     * @return Success or failure
     */
    bool extractSetQuaternionTrajectories(const char* dir_or_file_path,
     ...
     * @return Success or failure
     */
    bool extractSetCartCoordDemonstrationGroups(const char* in_data_dir_path,
     ...
     * @return Success or failure
     */
    bool extract3DPoints(const char* file_path, Points& points, bool is_comma_separated=false);
    ...
    /**
     * NON-REAL-TIME!!!\n
     * Sorry for the mess here, C++ compiler did NOT allow me to write the following function definition
     * in DataIO.cpp file (if I do, I have to instantiate it, which is annoying).
     *
     * @param dir_path Directory path containing the file onto where the matrix_data_structure will be written
     * @param file_name Name of the file inside dir_path onto where the matrix_data_structure will be written
     * @param matrix_data_structure The matrix data structure that will be written to the file
     * @return Success or failure
     */
    template <class T>
    bool writeMatrixToFile(const char* dir_path, const char* file_name, T const& matrix_data_structure)
    {
        ...
        return (rt_assert(DataIO::writeMatrixToFile(file_path, matrix_data_structure)));
    }
     ...
     * in DataIO.cpp file (if I do, I have to instantiate it, which is annoying).
     *
     * @param dir_path Directory path containing the file from where the content of matrix_data_structure will be read
     * @param file_name Name of the file inside dir_path from where the content of matrix_data_structure will be read
     * @param matrix_data_structure The matrix data structure that will be filled in \n
     *                              (assuming size is already fixed and known before calling this function)
     * @return Success or failure
     */
    template <class T>
    bool readMatrixFromFile(const char* dir_path, const char* file_name, T& matrix_data_structure)
    {
        if (rt_assert(DataIO::isValid()) == false)
        {
            return false;
        ...
        return (rt_assert(DataIO::readMatrixFromFile(file_path, matrix_data_structure)));
    }
     ...
     * in DataIO.cpp file (if I do, I have to instantiate it, which is annoying).
     *
     * @param file_path The complete file path onto where the matrix_data_structure will be written
     * @param matrix_data_structure The matrix data structure that will be written to the file
     * @return Success or failure
     */
    template <class T>
    bool writeMatrixToFile(const char* file_path, T const& matrix_data_structure)
    {
        if (rt_assert(DataIO::isValid()) == false)
        {
            return false;
     ...
     *
     * @param file_path The complete file path from where the content of matrix_data_structure will be read
     * @param matrix_data_structure The matrix data structure that will be filled in \n
     *                              (assuming size is already fixed and known before calling this function)
     * @return Success or failure
     */
    ...
    bool readMatrixFromFile(const char* file_path, T& matrix_data_structure)
    {
        ...
        unsigned long   alloc_1D_size   = matrix_data_structure.rows() * matrix_data_structure.cols();
     ...
     *
     * @param file_path The complete file path from where the content of matrix_data_structure will be read
     * @param matrix_data_structure The smart pointer to the matrix data structure that will be filled in \n
     *                              (size is unknown and dependent on the file being read, \n
     *                               after content is read from file, the space is allocated in the heap \n
     *                               for this matrix)
     * @return Success or failure
     */
    template <class T>
    bool readMatrixFromFile(const char* file_path, std::shared_ptr<T>& matrix_data_structure)
    {
        ...
        if (rt_assert(allocateMemoryIfNonRealTime(is_real_time, matrix_data_structure,
                                                  rows, cols)) == false)
        {
            return false;
        }
     ...
     * in DataIO.cpp file (if I do, I have to instantiate it, which is annoying).
     *
     * @param dir_path Directory path onto where the data will be saved (in the form of a file)
     * @param file_name Name of the file inside dir_path onto where the data will be saved
     * @param row_size The total number of rows of data_collection_buffer that will be saved
     * @param col_size The total number of columns of data_collection_buffer that will be saved
     * @param data_saving_buffer Data buffer whose content will be redirected to the file
                            ...
                            T1& data_saving_buffer, T2& data_collection_buffer)
    {
        ...
        if (rt_assert(DataIO::writeMatrixToFile(dir_path, file_name, data_saving_buffer)) == false)
        {
            return false;
        }
