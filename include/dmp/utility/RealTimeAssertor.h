 ...
 * is evaluated as 'false', then spawn-off a non-real-time thread that will
 * print off the error message onto a file.
 */
...
class RealTimeAssertor {
 ...
 public:
  ...
  /**
   * Empty out the real-time-error-log file specified in the RealTimeAssertor
   * class (for refilling).
   */
   ...
   * error_string to be appended
   */
  void appendStringTo_rt_err_file(
 ...
 *                               whose appendStringTo_rt_err_file() function is
 * to be called
 * @param err_str_container The ErrorStringContainer struct, which is \n
 *                          the input to the appendStringTo_rt_err_file()
 * function
 */
 ...
 * Thus this will spawn-off a non-real-time thread that will append the error
 * message onto the file specified in the RealTimeAssertor class.
 *
