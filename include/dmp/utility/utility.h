...
namespace dmp
{
     ...
     * Checks whether an Eigen data structure contains a NaN (Not-a-Number) element or not.
     *
     * @param input_mat Matrix whose elements are to be checked as being a NaN or not
    ...
    bool pseudoInverse(const T& input_mat, T& output_mat)
    {
        ...
        double tolerance    = epsilon * std::max(input_mat.cols(), input_mat.rows()) * svd.singularValues().array().abs()(0);

        output_mat          = svd.matrixV() * (svd.singularValues().array().abs() > tolerance).select(svd.singularValues().array().inverse(), 0).matrix().asDiagonal() * svd.matrixU().adjoint();
        return true;
    }
     ...
     *
     * @param smart_pointer The smart pointer, whose memory is assumed to be allocated on the heap
     * @param row_size The total number of rows of the allocated Eigen matrix which are instantiated
     * @param col_size The total number of columns of the allocated Eigen matrix which are instantiated
     * @return Success or failure
     */
    template <class T>
                           ...
                           uint row_size, uint col_size)
    {
        ...
        if ((smart_pointer->rows() != row_size) || (smart_pointer->cols() != col_size))
        {
            return false;
        }
        return true;
     ...
     * on the heap for the smart pointer. If called from real-time mode, this function
     * will return failure. Special for Eigen data structures' smart pointers.
     *
     * @param is_real_time Flag indicating whether the function calling this function is real-time or not
     * @param smart_pointer The smart pointer, whose memory will be allocated on the heap
     * @param row_size The total number of rows of the allocated Eigen matrix which are instantiated
     * @param col_size The total number of columns of the allocated Eigen matrix which are instantiated
     * @return Success or failure
     */
    ...
    {
        // input checking:
        // (we are only allowed to allocate memory on heap if we operate in NON-real-time mode!!!)
        ...
        if (T::MaxRowsAtCompileTime != (-1)) // if max #rows of matrix T is PRE-DEFINED:
        {
        ...
        if (T::MaxColsAtCompileTime != (-1)) // if max #cols of matrix T is PRE-DEFINED:
        {
     ...
     *
     * @param smart_pointer The smart pointer, whose memory is assumed to be allocated on the heap
     * @param size The total number of elements of the allocated std::vector data structure which are instantiated
     * @return Success or failure
     */
    template <class T>
    bool isMemoryAllocated(const std::shared_ptr<T>& smart_pointer,
     ...
     * IF this function is called at non-real-time mode, it will allocate a memory
     * on the heap for the smart pointer. If called from real-time mode, this function
     * will return failure. Special for std::vector data structure's smart pointers.
     *
     * @param is_real_time Flag indicating whether the function calling this function is real-time or not
     * @param smart_pointer The smart pointer, whose memory will be allocated on the heap
     * @param size The total number of elements of the allocated std::vector data structure which are instantiated
