 ...
 *
 *  Compilation of utility functions for Cartesians (consists of Cartesian
...
// REAL-TIME REQUIREMENTS
/**
 * Convert the representation of a pose state (position and orientation)
 * coordsys3 (e.g. robot end-effector)
 * -- currently represented w.r.t. coordsys2 (e.g. robot base) --,
 * into its representation w.r.t. coordsys1 (e.g. a Vicon-tracked object, fixed
 * w.r.t. robot base). Assumption: T_coordsys1_to_coordsys2 is constant/fixed in
 * time.
 * @param T_coordsys1_to_coordsys2 Input relative pose of coordsys2 seen
 * from/w.r.t. coordsys1 (assumed constant/fixed in time)
 * @param cart_coord_state_coordsys2_to_coordsys3 Input position state of
 * coordsys3 w.r.t. coordsys2
 * @param quat_state_coordsys2_to_coordsys3 Input orientation state of coordsys3
 * w.r.t. coordsys2 (as quaternion)
 * @param cart_coord_state_coordsys1_to_coordsys3 Output position state of
 * coordsys3 w.r.t. coordsys1
 * @param quat_state_coordsys1_to_coordsys3 Output orientation state of
 * coordsys3 w.r.t. coordsys1 (as quaternion)
 * @return Success (true) or failure (false)
 */
 ...
 * (e.g. robot end-effector) relatively represented w.r.t. coordsys2 (e.g. robot
 * base);
 * >> T_coordsys1_to_coordsys2: Relative pose of coordsys2 (e.g. robot base)
 * w.r.t. coordsys1 (e.g. a Vicon-tracked object, constant/fixed in time w.r.t.
 * robot base);
 * >> T_coordsys3_to_coordsys4: Relative pose of coordsys4 (e.g. tool)
 * w.r.t. coordsys3 (e.g. robot end-effector; tool is constant/fixed in time
 * w.r.t. robot end-effector); Compute the relative pose of coordsys4 (e.g.
 * tool) w.r.t. coordsys1 (e.g. a Vicon-tracked object). Assumption:
 * T_coordsys1_to_coordsys2 and T_coordsys3_to_coordsys4 are constant/fixed in
 * time.
 * @param T_coordsys1_to_coordsys2 Input relative pose of coordsys2 (e.g. robot
 * base) \n w.r.t. coordsys1 (e.g. a Vicon-tracked object, constant/fixed in
 * time w.r.t. robot base)
 * @param cart_coord_state_coordsys2_to_coordsys3 Input position state of
 * coordsys3 w.r.t. coordsys2
 * @param quat_state_coordsys2_to_coordsys3 Input orientation state of coordsys3
 * w.r.t. coordsys2 (as quaternion)
 ...
 * \n w.r.t. coordsys3 (e.g. robot end-effector; tool is constant/fixed in time
 * w.r.t. robot end-effector)
 * @param cart_coord_state_coordsys1_to_coordsys4 Output position state of
 * coordsys4 w.r.t. coordsys1
 * @param quat_state_coordsys1_to_coordsys4 Output orientation state of
 * coordsys4 w.r.t. coordsys1 (as quaternion)
 * @return Success (true) or failure (false)
 */
bool performStateCoordTransformation(
    Matrix4x4 T_coordsys1_to_coordsys2,
