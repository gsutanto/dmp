 ...
 *  ('general' means being used across many different places within this DMP
 * package). Limits of parameters, such as maximum DMP dimensions, maximum task
 * servo rate, maximum trajectory size, maximum DMP basis functions model size,
 * etc. are important in determining the size of some data structures.
 *
...
#define MAX_TASK_SERVO_RATE \
  1260  // assuming no robot running this DMP package has task servo rate
        // GREATER THAN <this value> Hertz
...
typedef Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::AutoAlign,
                      MAX_DMP_NUM_DIMENSIONS, MAX_MODEL_SIZE>
    MatrixNxM;
typedef Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::AutoAlign,
...
#define ZeroMatrixNxM(N, M)                                               \
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::AutoAlign, \
                MAX_DMP_NUM_DIMENSIONS, MAX_MODEL_SIZE>::Zero(N, M)
#define ZeroMatrix3xM(M)                                        \
  Eigen::Matrix<double, 3, Eigen::Dynamic, Eigen::AutoAlign, 3, \
                MAX_MODEL_SIZE>::Zero(3, M)
#define ZeroMatrix3xU(U)                                        \
  Eigen::Matrix<double, 3, Eigen::Dynamic, Eigen::AutoAlign, 3, \
                MAX_NUM_UNROLL_TRAJ_STATES>::Zero(3, U)
...
#define EyeMatrixNxN(N)                                                      \
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::AutoAlign,    \
                MAX_DMP_NUM_DIMENSIONS, MAX_DMP_NUM_DIMENSIONS>::Identity(N, \
                                                                          N)
#define EyeMatrix3x3 Eigen::Matrix3d::Identity()
#define EyeMatrix4x4 Eigen::Matrix4d::Identity()
#define ZeroVectorN(N)                                       \
  Eigen::Matrix<double, Eigen::Dynamic, 1, Eigen::AutoAlign, \
                MAX_DMP_NUM_DIMENSIONS>::Zero(N)
#define ZeroVectorM(M)                                       \
  Eigen::Matrix<double, Eigen::Dynamic, 1, Eigen::AutoAlign, \
                MAX_MODEL_SIZE>::Zero(M)
#define ZeroVector3 Eigen::Vector3d::Zero()
#define ZeroVector4 Eigen::Vector4d::Zero()
#define RandomVector3 \
  Eigen::Vector3d::Zero().unaryExpr(std::ptr_fun(sample_normal_distribution))
#define ZeroRowVector3 Eigen::Vector3d::Zero().transpose()
#define OnesVectorN(N)                                       \
  Eigen::Matrix<double, Eigen::Dynamic, 1, Eigen::AutoAlign, \
                MAX_DMP_NUM_DIMENSIONS>::Ones(N)
#define OnesVectorM(M)                                       \
  Eigen::Matrix<double, Eigen::Dynamic, 1, Eigen::AutoAlign, \
                MAX_MODEL_SIZE>::Ones(M)
