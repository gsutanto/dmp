...
class CartesianCoordDMP : public DMPDiscrete {
 protected:
  ...
  // (a) from local to global coordinate system (or local as seen from global
  // coordinate system)
  Matrix4x4 ctraj_hmg_transform_local_to_global_matrix;
  // (b) from global to local coordinate system (or global as seen from local
  // coordinate system) (inverse of ctraj_hmg_transform_matrix_global_to_local)
  Matrix4x4 ctraj_hmg_transform_global_to_local_matrix;

  // 5 critical states of a Cartesian coordinate trajectory (continuously being
  // updated/kept-track-of during trajectory reproduction/ unrolling by DMP):
  // start state, current state, current goal state, approaching-steady-state
  // goal state, and steady-state goal state: (i)  on the global coordinate
  // system
  ...
  Vector3 mean_start_global_position;  // average start position of the training
                                       // set of trajectories, in global
                                       // coordinate system
  Vector3 mean_goal_global_position;   // average goal position of the training
                                       // set of trajectories, in global
                                       // coordinate system

  Vector3
      mean_start_local_position;  // average start position of the training set
                                  // of trajectories, in local coordinate system
  Vector3
      mean_goal_local_position;  // average goal position of the training set of
                                 // trajectories, in local coordinate system
 ...
 public:
   ...
   * transformation system\n _SCHAAL_DMP_: original formulation by Schaal, AMAM
   * 2003 (default formulation_type)\n _HOFFMANN_DMP_: formulation by Hoffmann
   * et al., ICRA 2009
   ...
   * debugging
   * @param ctraj_local_coordinate_frame_selection Trajectory local coordinate
   * frame that will be used as the reference frame. Possible values are:\n 0:
   * No transformation, means local coordinate frame's axes is exactly in the
   * same direction as the global coordinate frame's axes\n 1: G. Sutanto's
   * trajectory local coordinate system formulation:\n Local x-axis is the
   * normalized vector going from start to goal position\n This method is
   * designed to tackle the problem on \n Schaal's trajectory local coordinate
   * system formulation (ctraj_local_coordinate_frame_selection == 2), which at
   * certain cases might have a discontinuous local coordinate transformation as
   * the local x-axis changes with respect to the global coordinate system, due
   * to the use of hard-coded IF statement to tackle the situation where local
   * x-axis gets close as being parallel to global z-axis.\n G. Sutanto's
   * trajectory local coordinate system formulation tackle this problem by
   * defining 5 basis vectors as candidates of anchor local z-axis at 5
   * different situations as defined below:\n
   ...
   *    Local x-axis is the normalized vector going from start to goal
   * position\n
   *    >> IF   local x-axis is "NOT TOO parallel" to the global z-axis\n
   *            (checked by: if dot-product or projection length is below some
   * threshold)\n THEN local x-axis and global z-axis (~= local z-axis) become
   * the anchor axes,\n local y-axis is the cross-product between them (z X x)\n
   *    >> ELSE IF local x-axis is "TOO parallel" to the global z-axis\n
   *       THEN    local x-axis and global y-axis (~= local y-axis) become the
   ...
   *    Local x-axis is the normalized vector approaching steady-state goal
   * position\n "Proposed" local y-axis is the normalized vector going from
   * start \n to a point "near" goal position (along local x-axis) such that
   ...
   *       THEN    (follow a similar method as Schaal's trajectory local
   * coordinate system formulation:)\n
   *       >> IF   local x-axis is "NOT TOO parallel" to the global z-axis\n
   *          THEN local x-axis and global z-axis (~= local z-axis) become the
   * anchor axes,\n local y-axis is the cross-product between them (z X x)\n
   *       >> ELSE IF local x-axis is "TOO parallel" to the global z-axis\n
   *          THEN    local x-axis and global y-axis (~= local y-axis) become
   * the anchor axes,\n local z-axis is the cross-product between them (x X y)\n
   ...
   *
   * @return Cartesian Coordinate DMP is valid (true) or Cartesian Coordinate
   * DMP is invalid (false)
   */
  bool isValid();
   ...
   *
   * @param in_data_dir_path Input data directory path, containing sample
   * trajectories, grouped by folder.\n A single CartesianCoordDMP movement
   * primitive will be learnt from each folder.
   * @param out_data_dir_path Output data directory path, where \n
   *                          the learnt CartesianCoordDMP movement primitives
   * parameters \n (weights and learning amplitude) will be saved into.\n If
   * (save_unroll_traj_dataset==true), the unrolled trajectories will also\n
   *                          saved in this directory path.
   * @param robot_task_servo_rate Robot's task servo rate
   * @param in_data_subdir_subpath If each folder UNDER in_data_dir_path
   * containing \n another sub-directories where the sample trajectories set are
   * actually saved,\n specify the sub-path here.
   * @param save_unroll_traj_dataset Choose whether to try unrolling \n
   *                                 the learnt CartesianCoordDMP movement
   * primitives (true) or not (false).
   * @param tau_unroll Tau for movement primitives unrolling
   * @return Success or failure
   */
  bool learnMultiCartCoordDMPs(const char* in_data_dir_path,
                               const char* out_data_dir_path,
                               double robot_task_servo_rate,
                               const char* in_data_subdir_subpath = "",
                               bool save_unroll_traj_dataset = false,
                               double tau_unroll = 1.0);
   ...
   * trajectory/vector of DMPStates.\n All are defined with respect to the
   * global coordinate system.
   * @param tau_init Time parameter that influences the speed of the execution
   ...
   * updated here. Otherwise you are responsible for updating it yourself.
   * @param result_dmp_state_global Computed next DMPState on the global
   * coordinate system (to be computed/returned)
   ...
   * @param result_dmp_state_global Computed next DMPState on the global
   * coordinate system (to be computed/returned)
   * @param result_dmp_state_local Computed next DMPState on the local
   * coordinate system (to be computed/returned)
   * @param transform_sys_forcing_term (optional) If you also want to know \n
   *                                   the transformation system's forcing term
   * value of the next state, \n please put the pointer here
  ...
  /**
   * Returns current goal position on x, y, and z axes on the global coordinate
   * system.
   *
   * @param current_goal_global Current goal position on the global coordinate
  ...
  /**
   * Returns steady-state goal position vector (G) on the global coordinate
   * system.
   *
   * @param Steady-state goal position vector (G) on the global coordinate
   * system (to be returned)
   * @return Success or failure
   */
   ...
   *
   * @param new_G_global New steady-state Cartesian goal position (G) on the
   * global coordinate system
   * @param new_approaching_G_global New approaching-steady-state Cartesian goal
   * position on \n the global coordinate system. Need to be specified correctly
   * \n when using ctraj_local_coord_selection = 3, otherwise can be \n
   *                                 specified arbitrarily (see more
  ...
  /**
   * Convert DMPState representation from local coordinate system into global
   * coordinate system.
   *
   * @param dmp_state_local DMPState representation in the local coordinate
   * system
   * @param dmp_state_global DMPState representation in the global coordinate
   * system
   * @return Success or failure
   */
  bool convertToGlobalDMPState(const DMPState& dmp_state_local,
                               DMPState& dmp_state_global);

  /**
   * Convert DMPState representation from global coordinate system into local
   * coordinate system.
   *
   * @param dmp_state_global DMPState representation in the global coordinate
   * system
   ...
   * @return Success or failure
   */
  bool convertToLocalDMPState(const DMPState& dmp_state_global,
   ...
   * @param file_name_mean_start_position_global Name of the file onto which the
   * mean start position w.r.t. global coordinate system will be saved
   * @param file_name_mean_goal_position_global Name of the file onto which the
   * mean goal position w.r.t. global coordinate system will be saved
   * @param file_name_mean_tau Name of the file onto which the mean tau will be
   * saved
   * @param file_name_mean_start_position_local Name of the file onto which the
   * mean start position w.r.t. local coordinate system will be saved
   * @param file_name_mean_goal_position_local Name of the file onto which the
   * mean goal position w.r.t. local coordinate system will be saved
   * @param file_name_ctraj_hmg_transform_local_to_global_matrix Name of the
   * file onto which the homogeneous coordinate transform matrix from local to
   * global coordinate systems will be saved
   * @param file_name_ctraj_hmg_transform_global_to_local_matrix Name of the
   * file onto which the homogeneous coordinate transform matrix from global to
   * local coordinate systems will be saved
   * @return Success or failure
   */
  bool saveParams(
      const char* dir_path,
