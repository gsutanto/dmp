 ...
 * TauSystem.h
 *
 *  Class defining computation of tau parameter for canonical system,
 * transformation system, and goal changer, which sometime is coupled with an
 * external variable
 *
...
class TauSystem {
 ...
 public:
  ...
  /**
   * @param tau_base_init Basic tau value (without any addition of coupling
   * terms)
   * @param real_time_assertor Real-Time Assertor for troubleshooting and
   * debugging
   * @param tau_ref Reference tau value:\n
   *                all computations which involve tau, is relative to this
   * value
   * @param tau_couplers All couplings associated with this tau system
   * (please note that this is a pointer to vector of pointer to TauCoupling
   * data structure, for flexibility and re-usability)
   */
  ...
  /**
   * @param tau_base_new New basic tau value (without any addition of coupling
  ...
  /**
   * @param tau_base Basic tau value (without any addition of coupling terms, to
   * be returned)\n (usually this is the time-length of the trajectory)
   * @return Success or failure
   */
  ...
  /**
   * @param tau_relative Relative tau value (to be computed and returned by this
   * function),\n i.e. time parameter that influences the speed of the
   ...
   * accumulated coupling term. Otherwise it will return 0.0.
   *
   * @param accumulated_ctau The accumulated tau system coupling term value (to
   * be returned)
   * @return Success or failure
   */
