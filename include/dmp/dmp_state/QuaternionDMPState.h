 ...
 * space dynamic movement primitives,” in IEEE International Conference on
 * Robotics and Automation (ICRA), 2014, pp. 2997–3004. [2] P. Pastor, L.
 * Righetti, M. Kalakrishnan, and S. Schaal, “Online movement adaptation based
 *      on previous sensor experiences,” in IEEE International Conference on
 * Intelligent Robots and Systems (IROS), 2011, pp. 365–371. [3] A.J. Ijspeert,
 * J. Nakanishi, H. Hoffmann, P. Pastor, and S. Schaal; Dynamical movement
 * primitives: Learning attractor models for motor behaviors; Neural Comput. 25,
 * 2 (February 2013), 328-373
 *
...
class QuaternionDMPState : public DMPState {
 ...
 public:
  ...
  /**
   * @param Q_init Initialization value of 4-dimensional Quaternion "position"
   * vector
   * @param real_time_assertor Real-Time Assertor for troubleshooting and
   * debugging
   */
   ...
   * @param Q_init Initialization value of 4-dimensional Quaternion "position"
   * vector
   * @param omega_init Initialization value of 3-dimensional angular velocity
   * vector
   * @param omegad_init Initialization value of 3-dimensional angular
   * acceleration vector
   * @param real_time_assertor Real-Time Assertor for troubleshooting and
   * debugging
   */
  QuaternionDMPState(const Vector4& Q_init, const Vector3& omega_init,
   ...
   * @param Q_init Initialization value of 4-dimensional Quaternion "position"
   * vector
   * @param Qd_init Initialization value of 4-dimensional Quaternion "velocity"
   * vector
   * @param Qdd_init Initialization value of 4-dimensional Quaternion
   * "acceleration" vector
   * @param real_time_assertor Real-Time Assertor for troubleshooting and
   * debugging
   */
  QuaternionDMPState(const Vector4& Q_init, const Vector4& Qd_init,
   ...
   */
  QuaternionDMPState(const Vector4& Q_init, const Vector4& Qd_init,
                     const Vector4& Qdd_init, const Vector3& omega_init,
                     const Vector3& omegad_init,
                     RealTimeAssertor* real_time_assertor);
   ...
   */
  QuaternionDMPState(const Vector4& Q_init, const Vector3& omega_init,
                     const Vector3& omegad_init, double time_init,
                     RealTimeAssertor* real_time_assertor);
   ...
   */
  QuaternionDMPState(const Vector4& Q_init, const Vector4& Qd_init,
                     const Vector4& Qdd_init, double time_init,
                     RealTimeAssertor* real_time_assertor);
  ...
  /**
   * @param other_quaterniondmpstate Another QuaternionDMPState whose value will
   * be copied into this QuaternionDMPState
   */
  QuaternionDMPState(const QuaternionDMPState& other_quaterniondmpstate);

  /**
   * Checks whether the dimensions of Q, Qd, and Qdd are all equivalent to
   * dmp_num_dimensions (4) and time is greater than or equal to 0.0.
   *
   * @return QuaternionDMPState is valid (true) or QuaternionDMPState is invalid
   * (false)
   */
  bool isValid() const;
