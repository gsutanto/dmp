...
%             (a) Euclidean (non-Quaternion-preserving) Integration 
%                 (1st Method, requiring additional re-normalization step 
%                  in order to retain validity of the Quaternions)
%             (b) Quaternion-preserving Integration
%                 (2nd Method, the Recommended Method)
...
% Euclidean (non-Quaternion-preserving) Integration (1st Method, 
% requiring additional re-normalization step in order to retain validity 
% of the Quaternions):
for i=1:traj_length
    ...
    % Euclidean (non-Quaternion-preserving) Integration:
    omega               = omega + (dt * omegad);
    quat                = quat + (dt * quatd);
    quat                = quat/norm(quat);  % Re-normalization to preserve validity of the Quaternion
...
% Quaternion-preserving Integration (2nd Method, the Recommended Method):
for i=1:traj_length
    quat_err_from_goal  = -computeQuatError(G_quat, quat);
    tau_omegad          = alpha_z * eye(3) * ((beta_z * eye(3) * quat_err_from_goal) - omega);
    omegad              = tau_omegad/tau;
    ...
    % Quaternion-preserving Integration:
...
for d=1:D
    subplot(D,1,d);
        hold on;
            ...
            if (d==1)
                legend('non-Quaternion-preserving', 'Quaternion-preserving');
            end
        hold off;
end
