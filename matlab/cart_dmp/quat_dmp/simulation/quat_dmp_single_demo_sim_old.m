...
cart_Q_traj_demo    = dlmread('../../../../data/cart_dmp/cart_dmp_wiggling/sample_quat_traj_recorded_demo_wiggling.txt');
time        = cart_Q_traj_demo(:,1)';
QT          = cart_Q_traj_demo(:,2:5)';
QdT         = cart_Q_traj_demo(:,6:9)';
QddT        = cart_Q_traj_demo(:,10:13)';
...
Q_hist                  = zeros(4, N_tau_multiplier*traj_length, N_tau_multiplier);
Qd_hist                 = zeros(4, N_tau_multiplier*traj_length, N_tau_multiplier);
Qdd_hist                = zeros(4, N_tau_multiplier*traj_length, N_tau_multiplier);
F_run                   = zeros(3, N_tau_multiplier*traj_length, N_tau_multiplier);
...
dcp_quaternion_old('init', ID, n_rfs, num2str(ID), c_order);
dcp_quaternion_old('reset_state', ID, Q0);
dcp_quaternion_old('set_goal', ID, QG, 1);

[w, F_target, F_fit] = dcp_quaternion_old('batch_fit', ID, tau, dt, QT, QdT, QddT);
...
for tau_multiplier = 1:N_tau_multiplier
    ...
    for i=1:(traj_length*tau_multiplier)
        [Q, Qd, Qdd, f] = dcp_quaternion_old('run', ID, (tau*tau_multiplier), dt);
...
for d=1:D
    subplot(D,1,d);
        hold on;
            ...
            title(['Forcing Term: target vs fit vs run, dimension ', num2str(d)]);
            legend('target','fit','run');
        hold off;
end
...
%             ylim([-1 1])
            ...
            title(['Q: target/demo vs run, dimension ', num2str(d)]);
            legend('target/demo','run');
        hold off;
end
...
for d=1:Dq
    subplot(Dq,1,d);
        hold on;
            ...
            title(['Qd: target/demo vs run, dimension ', num2str(d)]);
            legend('target/demo','run');
        hold off;
end
...
for d=1:Dq
    subplot(Dq,1,d);
        hold on;
            ...
            title(['Qdd: target/demo vs run, dimension ', num2str(d)]);
            legend('target/demo','run');
        hold off;
end
