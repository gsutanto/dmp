...
gT_ox3_cell     = cell(size(obs_list,2),1);
...
for i=1:size(obs_list,2)
    [ X_gTruth_cell{i,1}, Yo_cell{i,1}, Ydo_cell{i,1}, Yddo_cell{i,1}, gT_ox3_cell{i,1}, gT_v3_cell{i,1}, Ct_SYNTH_cell{i,1} ] = constructSynthObsAvoidPointTraj( wi, w_SYNTH, sample_traj_2D(1,:), sample_traj_2D(end,:), obs_list(:,i), traj_length, dt, beta_grid, k_grid, c_order );
    X_gTruth_stacked    = [X_gTruth_stacked; X_gTruth_cell{i,1}];
    gT_ox3_stacked      = [gT_ox3_stacked; gT_ox3_cell{i,1}];
...
for i=1:size(obs_list,2)
    [ X_observed_cell{i,1}, T_ox3_cell{i,1}, T_v3_cell{i,1} ] = constructObsAvoidPointFeatMat2D( Yo_cell{i,1}, Ydo_cell{i,1}, obs_list(:,i), beta_grid, k_grid );
...
if (is_plot_gT_vs_obs_first)
    figure;
    hold on; subplot(2,2,1); plot([1:length(gT_ox3(:,1))], gT_ox3(:,1), 'b', [1:length(gT_ox3(:,1))], T_ox3(:,1), 'g'); legend('latent', 'observed'); title('d=1 (ox3, x-axis)'); hold off;
    hold on; subplot(2,2,2); plot([1:length(gT_ox3(:,1))], gT_ox3(:,2), 'b', [1:length(gT_ox3(:,1))], T_ox3(:,2), 'g'); legend('latent', 'observed'); title('d=2 (ox3, y-axis)'); hold off;
    hold on; subplot(2,2,3); plot([1:length(gT_ox3(:,1))], gT_v3(:,1), 'b', [1:length(gT_ox3(:,1))], T_v3(:,1), 'g'); legend('latent', 'observed'); title('d=1 (v3, x-axis)'); hold off;
    hold on; subplot(2,2,4); plot([1:length(gT_ox3(:,1))], gT_v3(:,2), 'b', [1:length(gT_ox3(:,1))], T_v3(:,2), 'g'); legend('latent', 'observed'); title('d=2 (v3, y-axis)'); hold off;
end
if (is_plot_gT_vs_obs_stacked)
    figure;
    hold on; subplot(2,2,1); plot([1:length(gT_ox3_stacked(:,1))], gT_ox3_stacked(:,1), 'b', [1:length(gT_ox3_stacked(:,1))], T_ox3_stacked(:,1), 'g'); legend('latent', 'observed'); title('d=1 (ox3, x-axis)'); hold off;
    hold on; subplot(2,2,2); plot([1:length(gT_ox3_stacked(:,1))], gT_ox3_stacked(:,2), 'b', [1:length(gT_ox3_stacked(:,1))], T_ox3_stacked(:,2), 'g'); legend('latent', 'observed'); title('d=2 (ox3, y-axis)'); hold off;
    hold on; subplot(2,2,3); plot([1:length(gT_ox3_stacked(:,1))], gT_v3_stacked(:,1), 'b', [1:length(gT_ox3_stacked(:,1))], T_v3_stacked(:,1), 'g'); legend('latent', 'observed'); title('d=1 (v3, x-axis)'); hold off;
    hold on; subplot(2,2,4); plot([1:length(gT_ox3_stacked(:,1))], gT_v3_stacked(:,2), 'b', [1:length(gT_ox3_stacked(:,1))], T_v3_stacked(:,2), 'g'); legend('latent', 'observed'); title('d=2 (v3, y-axis)'); hold off;
end