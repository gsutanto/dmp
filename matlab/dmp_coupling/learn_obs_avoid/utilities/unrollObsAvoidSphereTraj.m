function [ Yo, Ydo, Yddo, Ct, X, R_ox3, R_v3 ] = unrollObsAvoidSphereTraj( varargin )
    w_DMP           = varargin{1};
    w_Ct            = varargin{2};
    start           = varargin{3};
    goal            = varargin{4};
    ...
    R_ox3           = zeros(traj_length,nP*D);
    R_v3            = zeros(traj_length,nP*D);
    ...
    for i=1:traj_length
        ...
        x           = zeros(1, nP*N_loa_feat_vect_per_point);    % obst avoid Ct feature vector
        ...
        for pn = 1:nP
            ...
            R_ox3(i,(pn-1)*D+1:(pn-1)*D+D)  = ox3(:,1)';
            R_v3(i,(pn-1)*D+1:(pn-1)*D+D)   = v3(:,1)';
