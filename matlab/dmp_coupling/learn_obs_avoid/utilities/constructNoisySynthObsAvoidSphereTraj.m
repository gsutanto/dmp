function [ X_gT, Yo, Ydo, Yddo, L_ox3, L_v3, Ct ] = constructNoisySynthObsAvoidSphereTraj( w_DMP, w_Ct_SYNTH, start, goal, obs, traj_length, dt, c_order, obs_radius, loa_feat_method, loa_feat_param )
    global dcps;
    n_rfs           = size(w_DMP,1);
    D               = size(w_DMP,2);
    ...
    for i=1:traj_length
        ...
        X_gT(i,:)       = x_gT;
        ct              = X_gT(i,:) * w_Ct_SYNTH + 0*randn(size(ct));
        Ct(i,:)         = ct;
        for d=1:D
